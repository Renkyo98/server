...
int saacproto_ServerDispatchMessage( int fd , char *encoded, char *debugfun)
{
	...
	if( strcmp( funcname , "GMSERVERLIST" ) == 0 ){//接收到跨服指令
		int itype;
		char *msg;
		itype = saacproto_demkstr_int( saacproto.token_list[2] );//取得编号
		msg = saacproto_wrapStringAddr( saacproto_stringwrapper[3] , saacproto.workbufsize , saacproto_demkstr_string( saacproto.token_list[3] ));//取得内容
		saacproto_gm_shutdown_recv(fd, itype,msg);//整理待发回GMSV
		return 0;
	}
#endif
...
void saacproto_gm_shutdown_send(int fd,int itype,char* msg){
	...
	saacproto_Send( fd , saacproto.work );//发回GMSV
}
#endif
