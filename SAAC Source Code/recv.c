                                ...
                                int lock,char* opt , int mesgid)
{
    ...
    // Spock 2000/10/31
    //log( "Ä³¸¯ÅÍ ÀÐ±â: ºÎ°¡ ÀÛ¾÷ %s %s, ÇÁ·Î¼¼½º=%d\n", id,pas,process );
    // Spock end
}
...
void saacproto_ACCharList_recv( int ti,char* id,char* pas ,char* ip,char* mac1 ,char* mac2 ,char* mac3, int mesgid,int charlistflg)
{
    ...
    snprintf(buf, sizeof(buf), "%d", process);
    if( !is_game_server_login( ti ) ){
        saacproto_ACCharList_send( ti , FAILED , "°ÔÀÓ ·Î±×ÀÎ ºÒ°¡£¡" , mesgid );
        return;
    }

	if (strlen(id)==0 || strlen(pas)==0){
	  saacproto_ACCharList_send( ti , FAILED , "°èÁ¤/ºñ¹Ð¹øÈ£´Â ºñ¿öµÑ ¼ö ¾ø½À´Ï´Ù£¡" , mesgid );
	  return;
	}
...
void saacproto_ACLock_recv( int ti ,char* id,int lock,int mesgid )
{
  ...
  if (lock==2) {
		...
		if ( lockUser( getGSName( ti ) , id , "0" , lock , result,sizeof(result),
										retdata ,sizeof( retdata ) , "0" , "0") < 0 ) {
#endif
			log( "»ç¿ëÀÚ Àá±Ý: %s ½ÇÆÐ\n" , id );
		} else {
			log( "»ç¿ëÀÚ Àá±Ý: %s ¼º°ø\n" , id );
		}
  }
    // Spock end
...
{
    if( !is_game_server_login(ti) ){
        log( "¼­¹ö¿¡¼­ °èÁ¤ ¹ß°ß:%s »óÅÂ:%d\n",
             id, status );
        return;
    }

	if( status == 0 ){
       log( "»ç¿ëÀÚ %s Àº(´Â) %s ¿¡¼­ Àá±ÝµÇÁö ¾ÊÀ½£¡\n", id , getGSName( ti ) );
       saacproto_ACKick_recv( ti, id, 6, -1);
    } else {
        log( "»ç¿ëÀÚ %s Àº(´Â) %s ¿¡¼­ ÀÌ¹Ì Àá±ÝµÊ£¡\n", id , getGSName( ti ) );
#ifdef _WAEI_KICK
	...
	case 6:		//MEMORY UNLOCK; UNLOCK
		{
			...
			if( strcmp( result , SUCCESSFUL ) == 0 ){
				log( "°èÁ¤ %s Àá±Ý ÇØÁ¦ ¼º°ø£¡\n" , id );
			} else {
				log( "°èÁ¤ %s Àá±Ý ÇØÁ¦ ½ÇÆÐ£¡\n" , id );
				return;
			}
...
int UNlockM_UnlockPlayer( void)
{
	...
	for( i=0; i<MAXUNlockM; i++)	{
		...
		if( isLocked( UNlockM[i].PlayerId ) ){
			log( "Àá±Ý ÇØÁ¦ ´ë±â ÇÃ·¹ÀÌ¾î: %s ¾ÆÁ÷ Àá±Ý ÇÊ¿ä!!\n", UNlockM[i].PlayerId);
		}else{
			log( "Àá±Ý ÇØÁ¦ ´ë±â ÇÃ·¹ÀÌ¾î: %s ÀÌ¹Ì ÇØÁ¦µÊ!!\n", UNlockM[i].PlayerId);
		}
		reset_UNlockMPlayer( i);
		nums++;
	}
	//log( "Àá±Ý ÇØÁ¦ ´ë±â: ÃÑ %d ¸í »ç¿ëÀÚ ÇØÁ¦ ¿Ï·á !!\n", nums);
	return nums;
}
		...
		case 0:
			if(strlen(id)>0){
				if(sasql_del_lock(id)){
					log("°èÁ¤ Àá±Ý ÇØÁ¦ ¼º°ø:%s\n",id);
				}else{
					log("°èÁ¤ Àá±Ý ÇØÁ¦ ½ÇÆÐ£¡:%s\n",id);
				}
			}
			if(strlen(ip)>0){
				if(sasql_del_lock(ip)){
					log("IP Àá±Ý ÇØÁ¦ ¼º°ø:%s\n",ip);
				}else{
					log("IP Àá±Ý ÇØÁ¦ ½ÇÆÐ£¡:%s\n",ip);
				}
			}
			break;
		case 1:
			if(strlen(id)>0){
				if(!sasql_chehk_lock(id)){
					...
					log("°èÁ¤%sÀº(´Â) ÀÌ¹Ì Àá±ÝµÇ¾î ÀÖÀ½£¡\n",id);
				}
			}
			if(strlen(ip)>0){
				if(!sasql_chehk_lock(ip)){
					...
					log("IP%sÀº(´Â) ÀÌ¹Ì Àá±ÝµÇ¾î ÀÖÀ½£¡\n",id);
				}
			}
			break;
		default:
...
void saacproto_ItemPetLocked_recv( int fd, int clifd, char *id, char *safepasswd )
{
	...
	if(strcmp(data, "º¸¾È Àá±ÝÀÌ ¼º°øÀûÀ¸·Î ÇØÁ¦µÇ¾ú½À´Ï´Ù£¡")==0){
		saacproto_ItemPetLocked_send(fd, clifd, 1, data);
	}else if(strcmp(data, "º¸¾È Àá±Ý ÇØÁ¦ ºñ¹Ð¹øÈ£°¡ ¼³Á¤µÇ¾î ÀÖÁö ¾Ê½À´Ï´Ù. ¾ÈÀüÀ» À§ÇØ 6ÀÚ¸® ÀÌ»óÀÇ ºñ¹Ð¹øÈ£¸¦ ÀÔ·ÂÇÏ°í ¹Ýµå½Ã ±â¾ïÇØµÎ¼¼¿ä£¡")==0){
...
void saacproto_FormulateAutoPk_recv( int fd, int clifd, char *id, int point )
{
	...
	char data[256];
	if(pkpoint == -1){
		sprintf(data, "³­¹« Æ÷ÀÎÆ® %d À»(¸¦) SQL·Î ÀüÈ¯ÇÒ ¼ö ¾ø½À´Ï´Ù. °ü¸®ÀÚ¿¡°Ô ¹®ÀÇÇÏ¼¼¿ä£¡", point);
	}else{
		sprintf(data, "³­¹« Æ÷ÀÎÆ® %d À»(¸¦) SQL·Î ÀüÈ¯Çß½À´Ï´Ù. ÇöÀç ³­¹« Æ÷ÀÎÆ®´Â ÃÑ %d ÀÔ´Ï´Ù", point, pkpoint);
	}
...
void saacproto_gm_shutdown_recv(int fd,int itype, char* msg)
{
	...
	for(i=0;i<MAXCONNECTION;i++)	{
		if( is_game_server_login(i) ){
			saacproto_gm_shutdown_send(i, itype,msg);//·¢»ØGMSV
		}
	}
}
#endif
