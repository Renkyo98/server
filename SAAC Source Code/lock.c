...
#ifdef _LOCK_ADD_NAME
		log("게임 진입:디렉토리:char/0x%x 계정:%s 이름:%s 서버:%s\n", entry, cdkey, name, server);
#else
		log("게임 진입:디렉토리:%x 계정:%s 서버:%s\n", entry, cdkey, server);
#endif
...
int DeleteMemLock(int entry, char *cdkey, int *process)
{
	...
	log("메모리 정보 삭제 위치=%x 계정=%s ..", entry, cdkey);
...
int GetMemLockState(int entry, char *cdkey, char *result)
{
	...
	while (ln!=NULL) {
		if (ln->use != 0) {
			if (strcmp(ln->cdkey, cdkey)==0) {
				sprintf(result, "%s 은(는) %s 에서 잠긴 상태입니다.",cdkey, ln->server);
				return 1;
			}
		}
		ln=ln->next;
	}
	sprintf(result, "%s 은(는) 잠기지 않았습니다.", cdkey);
	return 0;
}
