...
char* SKILL_makeSkillCoolTimeString( Skill* skill, int charaindex, int skill_num )
{
	int skillindex=SKILL_getInt( skill,SKILL_IDENTITY);
	int Pskill=PROFESSION_SKILL_getskillArray( skillindex);
	snprintf( ITEM_statusStringBuffer, sizeof( ITEM_statusStringBuffer ), "%d", PROFESSION_COOL_TIME( charaindex, skill_num ) );
    return ITEM_statusStringBuffer;
}
#endif
...
int PROFESSION_COOL_TIME(int charaindex, int skill_num)
{
	...
	skillid = CHAR_getCharSkill( charaindex, skill_num);
	Pskillid = PROFESSION_SKILL_getskillArray( skillid);
	if( Pskillid == -1 )	return 0;

	skill_name = PROFESSION_SKILL_getChar( Pskillid, PROFESSION_SKILL_FUNCNAME);// 技能名称
	...
	else if( strcmp( skill_name, "PROFESSION_DARK_SEAL" ) == 0 )	//闇灵封印
