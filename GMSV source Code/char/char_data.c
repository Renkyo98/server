...
BOOL CHAR_isCannotMagicArea( int floor , int x, int y, int magicnum ){
	...
	for( i=0 ; i<CHAR_invareanum ; i++ ) {
		if( CHAR_invarea[i].kind == CHAR_CANNOTMAGICAREA && CHAR_invarea[i].floor == floor && CoordinateInRect( &CHAR_invarea[i].area,x,y ) ){
			int     j;
			for( j = 0;j < sizeof(CHAR_invarea[i].work) && CHAR_invarea[i].work[j] != -1; j ++ ){
...
int CHAR_PetLevelUp( int petindex )
{
	...
	// 仿件弁卞方月  区及仿件母丞
	fRand = (float)RAND( RankRandTbl[petrank].min, RankRandTbl[petrank].max ) * 0.01f;
...
int PETFUSION_FusionPetSub( int charaindex, int Subindex1, int Subindex2, int *work, int *skill)
{
	...
	if (i < 7){
		CHAR_talkToCli(charaindex, -1, "你的宠物身上有装备没卸下喔！", CHAR_COLORYELLOW);
		return 0;
	}
#endif
	if( EVOLUTION_getPetFusionCode( CHAR_getInt( Subindex1, CHAR_PETID) ) < 0 ){//根据enemybase1.txt表判定是否为可以融合宠物
		return 0;
	}
...
BOOL PETFUSION_DelPet( int toindex, int Mainindex, int Subindex1, int Subindex2, int flg)
{
	...
	for( i=0; i<3; i++)	{
		...
		if( j >= CHAR_MAXPETHAVE){
			...
			snprintf( msgbuf,sizeof( msgbuf), "扒匙林促%s。", CHAR_getChar( petindex[i], CHAR_NAME));
			CHAR_talkToCli( toindex, -1, msgbuf,  CHAR_COLORYELLOW);
				LogPet(			
					...
					CHAR_getInt( toindex,CHAR_X ),
					CHAR_getInt( toindex,CHAR_Y ),
					CHAR_getChar( petindex[i], CHAR_UNIQUECODE),
					petindex[i],
					toindex
				);
		}
...
int CHAR_PetLevelUpNoFame( int petindex ){
	...
	tgh = (float)tgh   * fRand + Param[2] * fRand;
	dex = (float)dex   * fRand + Param[3] * fRand;
	CHAR_setInt( petindex, CHAR_VITAL,CHAR_getInt( petindex, CHAR_VITAL ) + max(0, (int)vital) );
	CHAR_setInt( petindex, CHAR_STR,CHAR_getInt( petindex, CHAR_STR ) + max(0, (int)str) );
	CHAR_setInt( petindex, CHAR_TOUGH,CHAR_getInt( petindex, CHAR_TOUGH ) + max(0, (int)tgh) );
	CHAR_setInt( petindex, CHAR_DEX,CHAR_getInt( petindex, CHAR_DEX ) + max(0, (int)dex) );
	return 0;
}
