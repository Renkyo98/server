...
void FAMILY_Add(int fd, int meindex, char* message)// 成立家族
{
	...
	if(tmpflag == -1){
		lssproto_WN_send( fd, WINDOW_MESSAGETYPE_MESSAGE,
			WINDOW_BUTTONTYPE_OK,
			-1, -1,
			makeEscapeString( "\n寸脚篮 捞固 何练捞 粮犁钦聪促！\n何练阑 呕硼 饶 积己秦林技夸！", buf, sizeof(buf)));
   	return;
	}
	if(tmpflag == -2){
		lssproto_WN_send( fd, WINDOW_MESSAGETYPE_MESSAGE,
	...
	//print("%s %s %s %d %s %s %s %d %d\n", fmname, charname, charid, charlv, petname,petattr, fmrule, fmsprite, chargrano);
#ifdef _PERSONAL_FAME
#ifdef _NB_FAMILYBADGE
		saacproto_ACAddFM_send(acfd, fmname, charname, charid, charlv,petname, petattr, fmrule, fmsprite, chargrano,CHAR_getInt(meindex, CHAR_FAME), fmbadge,CONNECT_getFdid(fd));
#else
		saacproto_ACAddFM_send(acfd, fmname, charname, charid, charlv,petname, petattr, fmrule, fmsprite, chargrano,CHAR_getInt(meindex, CHAR_FAME), CONNECT_getFdid(fd));
#endif
...
#else
	saacproto_ACAddFM_send(acfd, fmname, charname, charid, charlv,petname, petattr, fmrule, fmsprite, chargrano, CONNECT_getFdid(fd));
#endif
	// 要求最新家族列表
	//saacproto_ACShowFMList_send( acfd );
...
void ACAddFM(int fd, int result, int fmindex, int index){
  ...
  if(result == 1){
		...
		CHAR_talkToCli(meindex,-1,"俺牢 扁咯档啊 0栏肺 汲沥登菌嚼聪促",CHAR_COLORYELLOW);
#endif
		lssproto_WN_send( fd, WINDOW_MESSAGETYPE_MESSAGE,
			 WINDOW_BUTTONTYPE_OK,
			 -1, -1,
			 makeEscapeString( "\n何练捞 汲赋 登菌嚼聪促\n老林老 捞郴肺 啊练 牢盔捞 10疙阑 给逞辨 版快 何练篮 磊悼栏肺 秦眉邓聪促。", buf, sizeof(buf)));
...
void FAMILY_Join(int fd, int meindex, char *message)
{
	...
	if (!CHAR_CHECKINDEX(meindex))	return;
	if ((CHAR_getWorkInt(meindex, CHAR_WORKPARTYMODE) != CHAR_PARTY_NONE) || (CHAR_getWorkInt(meindex, CHAR_WORKBATTLEMODE) != BATTLE_CHARMODE_NONE)) return;
   ...
   CHAR_setWorkInt(meindex, CHAR_WORKFMINDEXI, index);
	...
	//CONNECT_getFdid(fd));
	saacproto_ACJoinFM_send(acfd, fmname, fmindex, charname, charid, charlv,index, CHAR_getInt(meindex, CHAR_FAME), CONNECT_getFdid(fd));
#else   
	//print("charfdid:%d\n", CONNECT_getFdid(fd));
	saacproto_ACJoinFM_send(acfd, fmname, fmindex, charname, charid, charlv,index, CONNECT_getFdid(fd));
#endif
	CHAR_sendCToArroundCharacter(CHAR_getWorkInt(meindex, CHAR_WORKOBJINDEX));
}
  ...
  if(!CHAR_CHECKINDEX(meindex))	return;
   ...
   if (result == 1) {
		 lssproto_WN_send( fd, WINDOW_MESSAGETYPE_MESSAGE,
			WINDOW_BUTTONTYPE_OK,
			-1, -1,
			makeEscapeString( "\n啊涝 脚没捞 肯丰 登菌嚼聪促！\n铰牢阑 措扁秦林矫扁 官而聪促。", buf, sizeof(buf)));
...

	{
		...
		for( i=1; i<=MANORNUM; i++){
			...
			if(fmpks[fmpks_pos+1].host_index > -1 && fmpks[fmpks_pos+1].guest_index > -1){
				...
				if (CHAR_getInt(meindex, CHAR_FMLEADERFLAG) == 1){
#endif         
					if( (fmpks[fmpks_pos+1].host_index+1)  == CHAR_getInt(meindex, CHAR_FMINDEX) || (fmpks[fmpks_pos+1].guest_index+1) == CHAR_getInt(meindex, CHAR_FMINDEX) ){
						lssproto_WN_send( fd, WINDOW_MESSAGETYPE_MESSAGE,
			                             WINDOW_BUTTONTYPE_OK,
			                             -1, -1,
...
void ACLeaveFM( int fd, int result, int resultflag)
{
	...
	if (result == 1){
		...
		lssproto_WN_send( fd, WINDOW_MESSAGETYPE_MESSAGE,
			WINDOW_BUTTONTYPE_OK, -1, -1,
			makeEscapeString( "\n申何练捞 沥惑利栏肺 呕硼 登菌嚼聪促.", buf, sizeof(buf)));
...
#ifdef _NEW_MANOR_LAW
		CHAR_setInt(meindex,CHAR_MOMENTUM,0);
		CHAR_talkToCli(meindex,-1,"俺牢 扁技啊 0栏肺 汲沥登菌嚼聪促.",CHAR_COLORYELLOW);
#endif
...
void FAMILY_SetPoint(int fd, int meindex, char *message)
{
  ...
  if(fmdp < 200000){
   	lssproto_WN_send( fd, WINDOW_MESSAGETYPE_MESSAGE,WINDOW_BUTTONTYPE_OK,-1, -1,
		makeEscapeString( "\n你家族的声望不足2000声望，无法申请庄园！", buf, sizeof(buf)));
    return;
  }
#endif
#ifdef FAMLYNUM_CF
...
void ACSetFMPoint(int ret, int r, int clifd)
{
   ...
   if (ret == 0){
   		if (r == -1)
   			...
   			sprintf(message, "您的家族人数未达申请标准唷！最少必须拥有10个成员");//成员数在saac/acfamly.h里修改
   		else if (r == -6)
...
int FAMILY_RidePet( int fd, int meindex, char* message )
{
	...
	if( strcmp( token, "P") == 0) {
		...
		if( atoi(token2) != -1 ) {
			...
			if( CHAR_getInt( meindex, CHAR_LV)+RideLevel < CHAR_getInt( petindex, CHAR_LV )  ) 
			{ 
				//char buff[255];
...
//   	    CHAR_getInt(meindex, CHAR_FMLEADERFLAG) != FMMEMBER_VICELEADER)  return;
   	...
   	if (getStringFromIndexWithDelim(message, "|", 3, token2, sizeof(token2)) == FALSE)	return;
		...
		for( i=0 ; i<FMPOINTNUM ; i++ ) {
			...
			//print("调试输出:%d.%d\n",CHAR_getInt( meindex, CHAR_FMINDEX ),atoi(subtoken));
...
int Battle_RidePet( int meindex, int defNo, int cooltimeidx, int cooltime )
{
	...
	if( petno != -1 ){
		...
		if( CHAR_getInt( meindex, CHAR_DEFAULTPET ) == petno )	return -1;
		if( CHAR_getInt( meindex, CHAR_RIDEPET) != -1 ) return -1;
		if( CHAR_getInt( meindex, CHAR_LEARNRIDE) < CHAR_getInt( petindex, CHAR_LV )  ) return -1;	
		if( CHAR_getWorkInt( petindex, CHAR_WORKFIXAI ) < 100 )return -1;
		if( CHAR_getInt( meindex, CHAR_LV)+5 < CHAR_getInt( petindex, CHAR_LV )  ) return -1;
		
		CHAR_setInt(meindex,CHAR_RIDENAMEFLG,0);
		rideGraNo = FamilyRideFunction(meindex, petindex, petno);
		if( rideGraNo <= 0 ){
			CHAR_talkToCli( meindex, -1, "进入战斗时未设置骑宠，无法进行骑乘！", CHAR_COLORYELLOW );
			return -1;
		}
