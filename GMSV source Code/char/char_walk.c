							...
							if (entflag) {
								...
								if( getEqRandenemy( enfd) > 0 )	{
									...
									if( Rnum > RandEnemy )	{
										...
										//encounter!!
										cep = minep;
										lssproto_EN_recv(enfd,CHAR_getInt(charaindex,CHAR_X),CHAR_getInt(charaindex,CHAR_Y));
#ifdef _Item_MoonAct
									}
								}else{
										cep = minep;
		...
		for( j = y-seesiz ; j <= y+seesiz ;j ++ ) {
			...
			for( object = MAP_getTopObj(fl,i,j) ; object ; object = NEXT_OBJECT(object ) ){
				...
				if( whichtype == CHAR_TYPEPLAYER){
					...
					if( OBJECT_getType(objindex) == OBJTYPE_CHARA ) {
						if( CHAR_getInt( c_index, CHAR_WHICHTYPE ) == CHAR_TYPEPLAYER && CHAR_getWorkInt( c_index, CHAR_WORKPARTYMODE ) == CHAR_PARTY_LEADER ){//判断是否为队长
							if( CHAR_makeCAOPT1String( objindex, cabuf, sizeof( cabuf),	CHAR_ACTLEADER,1 )){//给予组队标志
								CONNECT_appendCAbuf( fd,cabuf,strlen(cabuf));
							}
						}
						if( (CHAR_getInt( c_index, CHAR_WHICHTYPE ) == CHAR_TYPEPLAYER) && CHAR_getWorkInt( charaindex, CHAR_WORKPARTYMODE ) == CHAR_PARTY_LEADER && CHAR_getWorkInt( c_index, CHAR_WORKBATTLEMODE) == BATTLE_CHARMODE_NONE){
							int		tofd = getfdFromCharaIndex( c_index );
							if( tofd != -1 ) {
								if( CHAR_makeCAOPT1String( CHAR_getWorkInt( charaindex, CHAR_WORKOBJINDEX),cabuf, sizeof( cabuf), CHAR_ACTLEADER,1 )){//给予组队标志
									CONNECT_appendCAbuf( tofd,cabuf,strlen(cabuf));
								}
							}
						}
						if( CHAR_getInt( c_index, CHAR_WHICHTYPE ) == CHAR_TYPEPLAYER && 
							CHAR_getWorkInt( c_index, CHAR_WORKBATTLEMODE )	!= BATTLE_CHARMODE_NONE ){//判断是否战斗中
							if( CHAR_getWorkInt( c_index, CHAR_WORKBATTLEWATCH) == TRUE ) {//判断是否观战
								if( CHAR_makeCAOPT1String( objindex, cabuf, sizeof( cabuf),
															CHAR_ACTBATTLEWATCH, 1)){//给予观战标志
									CONNECT_appendCAbuf( fd, cabuf, strlen( cabuf));
								}
							}else {
...
#endif
#ifdef _ANGEL_SUMMON
						if( CHAR_getInt( c_index, CHAR_WHICHTYPE ) == CHAR_TYPEPLAYER && CHAR_getWorkInt( c_index, CHAR_WORKANGELMODE ) == TRUE ){
