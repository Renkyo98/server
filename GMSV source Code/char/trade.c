	...
	case TRADE_ITEM:
		{
			...
			if( usTi < CHAR_STARTITEMARRAY || usTi >= CHAR_getMaxInventory( meindex ) ) return FALSE;
			itemindex = CHAR_getItemIndex( meindex, usTi );
			if( !ITEM_CHECKINDEX( itemindex))	return FALSE;

			if( ITEM_getInt( itemindex, ITEM_VANISHATDROP) == 1 ){
				char buf[256];
				sprintf( buf, "[%s](篮)绰 芭贰且 荐 绝绰 酒捞袍涝聪促.", ITEM_getChar( itemindex, ITEM_NAME) );
			...
			if (ITEM_getInt(itemindex,ITEM_ItemLock) == 1){
				char buf[256];
				sprintf( buf, "【温馨提示】%s 此道具为绑定道具，无法交易，请妥 票９堋?", ITEM_getChar( itemindex, ITEM_SECRETNAME) );
				CHAR_talkToCli( meindex, -1, buf, CHAR_COLORYELLOW);
				return	FALSE;
			}
#endif
...
#ifdef _LOCK_PET_ITEM
	{
...
#ifdef _PET_ITEM
			{
				...
				for( i = 0 ; i < CHAR_MAXPETITEMHAVE ; i ++ ){
					...
					if (ITEM_getInt(itemindex,ITEM_ItemLock) == 1){
						CHAR_talkToCli( meindex, -1, "宠物 砩嫌邪蠖ǖ牡谰呶薹ń灰祝?", CHAR_COLORYELLOW );
						return FALSE;
					}
				}
			}
...
BOOL TRADE_Search(int fd, int meindex, char* message)
{
   ...
   //  瓒ㄗ刺灰浊肭笾?//   CHAR_setWorkInt(meindex, CHAR_WORKTRADEMODE, CHAR_TRADE_SENDING);
   ...
   CHAR_getCoordinationDir(CHAR_getInt(meindex, CHAR_DIR),
	...
	if (cnt == 1){
		...
		if(CHAR_getInt(meindex, CHAR_LOCKED)==1 && getItemPetLocked() == 1){
			...
			return FALSE;
		}else if(CHAR_getInt(toindex, CHAR_LOCKED)==1 && getItemPetLocked()== 1){
			char message[256];
...

	{
		...
		for( i=0; i<5; i++){
			if( CHAR_CHECKINDEX( ToPet[i]) ){
				memset( buf3, 0, sizeof( buf3));
				sprintf( buf3, "%s[%s](%d),",
					CHAR_getChar( ToPet[i], CHAR_NAME), CHAR_getChar( ToPet[i], CHAR_UNIQUECODE),
			...
			if( CHAR_CHECKINDEX( MePet[i]) ){
				memset( buf3, 0, sizeof( buf3));
				sprintf( buf3, "%s[%s](%d),",
					CHAR_getChar( MePet[i], CHAR_NAME),CHAR_getChar( MePet[i], CHAR_UNIQUECODE),
...
#ifdef _PET_ITEM
{
	...
	TRADE_ChangeItem(meindex, toindex, a, d, item1, item4,itemindex1, itemindex4);
	TRADE_ChangeItem(meindex, toindex, b, e, item2, item5,itemindex2, itemindex5);
	TRADE_ChangeItem(meindex, toindex, c, f, pet3, pet6,itemindex3, itemindex6);
...
BOOL TRADE_CheckTradeList( int meindex, STradeList *temp1, int toindex, STradeList *temp2)
{
	...
	MeSurplus = CHAR_findSurplusItemBox( meindex);//郴啊 啊龙荐乐绰 酒捞袍沫捞 绝澜
	...
	if( MeSurplus < MeNeeds ){
		CHAR_talkToCli( meindex, -1, "酒捞袍 沫捞 啊垫 谩绢夸!", CHAR_COLORYELLOW);//郴 酒捞袍沫捞 啊垫谩澜
		CHAR_talkToCli( toindex, -1, "惑措规狼 酒捞袍 沫捞 啊垫 谩绢夸!", CHAR_COLORYELLOW);//惑措规 酒捞袍沫捞 啊垫谩澜
		return FALSE;
	}
	if( ToSurplus < ToNeeds ){
		CHAR_talkToCli( toindex, -1, "酒捞袍 沫捞 啊垫 谩绢夸!", CHAR_COLORYELLOW);//惑措规捞 好阑锭 郴 酒捞袍沫捞 啊垫谩澜
		CHAR_talkToCli( meindex, -1, "惑措规狼 酒捞袍 沫捞 啊垫 谩绢夸!", CHAR_COLORYELLOW);//郴 酒捞袍沫捞 啊垫 谩澜
		return FALSE;
	}
	...
	if( MeSurplus < MeNeeds ){
		CHAR_talkToCli( meindex, -1, "惑措狼 其飘 沫捞 啊垫 谩绢夸!", CHAR_COLORYELLOW);
		CHAR_talkToCli( toindex, -1, "其飘 沫捞 啊垫 谩绢夸!", CHAR_COLORYELLOW);
		return FALSE;
	}
	if( ToSurplus < ToNeeds ){
		CHAR_talkToCli( meindex, -1, "其飘 沫捞 啊垫 谩绢夸!", CHAR_COLORYELLOW);
		CHAR_talkToCli( toindex, -1, "惑措狼 其飘 沫捞 啊垫 谩绢夸!", CHAR_COLORYELLOW);
		return FALSE;
	}
	...
	MeNeeds = temp2->Golds;
	if( MeSurplus < MeNeeds ){
		CHAR_talkToCli( meindex, -1, "芭贰 陛咀捞 惑茄急俊 档崔窍看嚼聪促.", CHAR_COLORYELLOW);
		CHAR_talkToCli( toindex, -1, "芭贰 陛咀捞 惑茄急俊 档崔窍看嚼聪促.", CHAR_COLORYELLOW);
		return FALSE;
	}
	if( ToSurplus < ToNeeds ){
		CHAR_talkToCli( meindex, -1, "芭贰 陛咀捞 惑茄急俊 档崔窍看嚼聪促.", CHAR_COLORYELLOW);
