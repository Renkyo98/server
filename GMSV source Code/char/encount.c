...
#define		CREATEPROB_TOKEN	11
			{
				...
				if( checkRedundancy( ENCOUNT_table[encount_readlen].groupid, arraysizeof( ENCOUNT_table[encount_readlen].groupid))){
					fprint( "文件语法错误:%s 第%d行\n", filename,linenum);
					continue;
				}
			}
...
int ENCOUNT_getEncountPercentMin( int charaindex, int floor , int x, int y ){
	...
	if( ret != -1 ) {
		...
		if( CHAR_getWorkInt( charaindex, CHAR_WORK_TOHELOS_COUNT) > 0 ) {
			ret = ceil( ret * ((100 + CHAR_getWorkInt( charaindex, CHAR_WORK_TOHELOS_CUTRATE)) / 100.0));
		}
		if( ret < 0 ) ret = 0;
		if( ret > 100 ) ret = 100;
  }
