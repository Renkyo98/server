...
{		
			...
			for( i1 = 0 ; i1 < CHAR_MAXPETITEMHAVE ; i1 ++ ){
				...
				if( itemindex >= 0 ) {
					if (ITEM_getInt(itemindex,ITEM_ItemLock) == 1){
						char TM_SayBuff[64];
						snprintf(TM_SayBuff, sizeof(TM_SayBuff), "装备绑定道具的宠物无法丢出！");
  ...
  CHAR_setCharPet( charaindex, havepetindex, -1);
   ...
   snprintf( szPet, sizeof( szPet ), "[%s]其飘 [%d]饭骇 其飘啊 %d檬 饶俊 荤扼笼聪促。", CHAR_getChar(petindex,CHAR_NAME),CHAR_getInt(petindex,CHAR_LV),getPetdeletetime() );
...
int PET_createPetFromCharaIndex( int charaindex, int enemyindex)
{
    ...
    CharNew.data[CHAR_PETRANK]  = CHAR_getInt(enemyindex, CHAR_PETRANK);
		...
		CharNew.data[CHAR_CRIT_RATE] = CHAR_getInt(enemyindex, CHAR_CRIT_RATE);//会心率
	...
	{
	CHAR_setInt( newindex, CHAR_PETDATAH, CHAR_getWorkInt( enemyindex, CHAR_WORKMAXHP));
	CHAR_setInt( newindex, CHAR_PETDATAS, CHAR_getWorkInt( enemyindex, CHAR_WORKATTACKPOWER));
	CHAR_setInt( newindex, CHAR_PETDATAT, CHAR_getWorkInt( enemyindex, CHAR_WORKDEFENCEPOWER));
	CHAR_setInt( newindex, CHAR_PETDATAD, CHAR_getWorkInt( enemyindex, CHAR_WORKQUICK));
	}
#endif
