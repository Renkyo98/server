...
void CHAR_CHAT_DEBUG_delitem( int charindex , char *message )
{
	...
	if( strcmp( message, "all") == 0 ) {
		...
		if( equipplace < CHAR_STARTITEMARRAY || equipplace > CHAR_getMaxInventory( charindex ) -1 ) {
...
void CHAR_CHAT_DEBUG_reset( int charaindex, char* message )
{
	...
	if( i >= MAXMESS_LISTNUM ){
		CHAR_talkToCli( charaindex, -1, "舅荐绝绰疙飞绢涝聪促",  CHAR_COLORRED);//CHAR_COLORYELLOW
		return;
	}
	switch( i){
	case ALLRESET:
...
void CHAR_CHAT_DEBUG_petlevelup( int charaindex, char* message )
{
	...
	if(strlen(cdkey)>0){
		...
		CHAR_setInt( petindex , CHAR_HP , CHAR_getWorkInt( petindex , CHAR_WORKMAXHP ) );
		sprintf( token, "其飘锅龋 % d阑设%d 饭骇肺 函版窍看嚼聪促!", petid+1, CHAR_getInt( petindex, CHAR_LV));
		CHAR_talkToCli( charaindex, -1,token, CHAR_COLORYELLOW );
		CHAR_complianceParameter( petindex );
		snprintf( token, sizeof( token ), "K%d", petid );
		CHAR_sendStatusString( charaindex, token );
...
#ifdef _GM_KING
                snprintf( line, sizeof( line),
               		"管理员[%s]将 %s 禁言%d分钟。",CHAR_getChar(index,CHAR_NAME),
                	CHAR_getChar( index, CHAR_NAME),
                	minu );
#else
                snprintf( line, sizeof( line),
...
#ifdef FAMLYNUM_CF
		sprintf(szToken,"参数不正确1 [set_manor_owner 庄园id(1~%d) 使用旗标(0:名称 1:id) 家族名称或id]",getfamlynum());
#else
		sprintf(szToken,"参数不正确1 [set_manor_owner 庄园id(1~10) 使用旗标(0:名称 1:id) 家族名称或id]");
#endif
		CHAR_talkToCli(charindex,-1, szToken, CHAR_COLORRED);
		return;
	}
...
#ifdef FAMLYNUM_CF
		sprintf(szToken,"参数不正确2 [set_manor_owner 庄园id(1~%d) 使用旗标(0:名称 1:id) 家族名称或id]",getfamlynum());
#else
		sprintf(szToken,"参数不正确2 [set_manor_owner 庄园id(1~10) 使用旗标(0:名称 1:id) 家族名称或id]");
#endif
		CHAR_talkToCli(charindex,-1, szToken, CHAR_COLORRED);
		return;
	}
...
void CHAR_CHAT_DEBUG_set_schedule_time( int charindex, char* message )
{
	...
	if(message == NULL || getStringFromIndexWithDelim(message," ",1,szId,sizeof(szId)) == FALSE){
#ifdef FAMLYNUM_CF
		sprintf(szMsg,"参数不正确1 [set_schedule_time 庄园id(1~%d) 时间(分)]",getfamlynum());
#else
...
#ifdef FAMLYNUM_CF
		sprintf(szMsg,"参数不正确2 [set_schedule_time 庄园id(1~%d) 时间(分)]",getfamlynum());
#else
...
void CHAR_CHAT_DEBUG_DelJiaRen(int charaindex,char *message)
{
	...
	if(strstr(areabuf ,"all")!=NULL){
		...
		for(i=startindex ;i<=charmaxnum ;i++){
			if(CHAR_CHECKINDEX(i) == FALSE) continue;
			if( CHAR_getInt( i, CHAR_WHICHTYPE) != CHAR_TYPEPLAYERNPC ) continue;
		...
		for(j=x-area;j<=x+area;j++){
			for( object = MAP_getTopObj(floor,j,i) ; object ;object = NEXT_OBJECT(object ) ){
				...
				if( OBJECT_getType(o) == OBJTYPE_CHARA ){
					chara_index=OBJECT_getIndex(o);
					if(CHAR_CHECKINDEX(chara_index) == FALSE) continue;
					if( CHAR_getInt( chara_index, CHAR_WHICHTYPE) != CHAR_TYPEPLAYERNPC || CHAR_getWorkInt( chara_index, CHAR_WORKPARTYMODE) != CHAR_PARTY_LEADER ) continue;
					//if( chara_index == charaindex ) continue;  //找到自己了 
						getnum++;
						//snprintf( line, sizeof( line), "第%d个找到[%s]的帐号是[%s]",getnum,CHAR_getChar(chara_index, CHAR_NAME),CHAR_getChar(chara_index, CHAR_CDKEY));
            //CHAR_talkToCli( charaindex, -1,line, CHAR_COLORRED);	
...
void CHAR_CHAT_DEBUG_SERVERGM(int charindex , char *message ){
	...
	if(strstr(message ,"shutdown")!=NULL || strstr(message ,"关服")!=NULL){
		itype = 1;
	}else if(strstr(message ,"reset")!=NULL || strstr(message ,"重读设置")!=NULL){
		itype = 2;
	}else if(strstr(message ,"itemreload")!=NULL || strstr(message ,"重读道具")!=NULL){
		itype = 3;
	}else if(strstr(message ,"reloadcf")!=NULL || strstr(message ,"重读CF")!=NULL){
		itype = 4;
	}else if(strstr(message ,"reloadlua")!=NULL || strstr(message ,"重读LUA")!=NULL){
		itype = 5;
	}else if(strstr(message ,"gmreload")!=NULL || strstr(message ,"重读GM设置")!=NULL){
		itype = 6;
	}
	...
	if((itype <= 0) || (itype > 6)){
		...
		CHAR_talkToCli(charindex, -1, buf, CHAR_COLORWHITE);
		sprintf(buf,"%s","1、关服  2、重读设置 3、重读道具 4、重读CF 5、重读LUA 6、重读GM设置");
		CHAR_talkToCli(charindex, -1, buf, CHAR_COLORWHITE);
		return;
	}
	...
	if ((itype < 2) || (itype > 5)){
		if( strlen(token)==0){
			...
			CHAR_talkToCli(charindex, -1, buf, CHAR_COLORWHITE);
			sprintf(buf,"%s","关服命令需输入时间，2-4指令编号输入0 重读LUA输入路径");
			CHAR_talkToCli(charindex, -1, buf, CHAR_COLORWHITE);
			return;
		}
	}
	...
	if (itype == 1){
		CHAR_talkToCli( charindex, -1, "[跨服指令]发送[关服]指令成功！~",  CHAR_COLORRED);
	}else if (itype == 2){
		CHAR_talkToCli( charindex, -1, "[跨服指令]发送[重读设置]指令成功！~",  CHAR_COLORRED);
	}else if (itype == 3){
		CHAR_talkToCli( charindex, -1, "[跨服指令]发送[重读道具]指令成功！~",  CHAR_COLORRED);
	}else if (itype == 4){
		CHAR_talkToCli( charindex, -1, "[跨服指令]发送[重读CF]指令成功！~",  CHAR_COLORRED);
	}else if (itype == 5){
		CHAR_talkToCli( charindex, -1, "[跨服指令]发送[重读LUA]指令成功！~",  CHAR_COLORRED);
	}else if (itype == 6){
		CHAR_talkToCli( charindex, -1, "[跨服指令]发送[重读GM设置]指令成功！~",  CHAR_COLORRED);
	}
}
#endif
...
void CHAR_CHAT_DEBUG_setpoint(int charaindex, char *message){
	...
	if(strstr(itype ,"胶沛")!=NULL || atoi(itype)==1){
		...
		if (toindex != charaindex){
			sprintf( token, "[GM]%s丛膊辑 [%d]胶沛阑 焊郴林菌嚼聪促!", CHAR_getChar( charaindex, CHAR_NAME),getpoint);
			CHAR_talkToCli( toindex, -1, token, CHAR_COLORYELLOW );
			sprintf( token, "[%s]丛膊 [%d]胶沛阑 傈价窍看嚼聪促!", CHAR_getChar( toindex, CHAR_NAME),getpoint);
			CHAR_talkToCli( charaindex, -1,token, CHAR_COLORYELLOW );
		}else{
		...
		if (toindex != charaindex){
			sprintf( token, "[GM] %s 给你的声望增加%d!", CHAR_getChar( charaindex, CHAR_NAME),getpoint);
			CHAR_talkToCli( toindex, -1, token, CHAR_COLORYELLOW );
			sprintf( token, "玩家 %s 的声望增加%d!", CHAR_getChar( toindex, CHAR_NAME),getpoint);
			CHAR_talkToCli( charaindex, -1,token, CHAR_COLORYELLOW );
		}else{
		...
		FM_FIX_FMFEED,buf,"",CHAR_getWorkInt(toindex,CHAR_WORKFMCHARINDEX),CONNECT_getFdid(fd));
		...
		if (toindex != charaindex){
			sprintf( token, "[GM] %s 给你的活力增加%d!", CHAR_getChar( charaindex, CHAR_NAME),getpoint);
			CHAR_talkToCli( toindex, -1, token, CHAR_COLORYELLOW );
			sprintf( token, "玩家 %s 的活力增加%d!", CHAR_getChar( toindex, CHAR_NAME),getpoint);
			CHAR_talkToCli( charaindex, -1,token, CHAR_COLORYELLOW );
		}else{
		...
		if (toindex != charaindex){
			sprintf( token, "[GM]%s丛膊辑 [%d]瘤盔器牢飘甫 焊郴林菌嚼聪促!", CHAR_getChar( charaindex, CHAR_NAME),getpoint);
			CHAR_talkToCli( toindex, -1, token, CHAR_COLORYELLOW );
			sprintf( token, "[%s]丛膊 [%d]瘤盔器牢飘甫 傈价窍看嚼聪促!", CHAR_getChar( toindex, CHAR_NAME),getpoint);
			CHAR_talkToCli( charaindex, -1,token, CHAR_COLORYELLOW );
		}else{
		...
		if (toindex != charaindex){
			sprintf( token, "[GM] %s 给你的积分增加%d!", CHAR_getChar( charaindex, CHAR_NAME),getpoint);
			CHAR_talkToCli( toindex, -1, token, CHAR_COLORYELLOW );
			sprintf( token, "玩家 %s 的积分增加%d!", CHAR_getChar( toindex, CHAR_NAME),getpoint);
			CHAR_talkToCli( charaindex, -1,token, CHAR_COLORYELLOW );
		}else{
...
void CHAR_CHAT_DEBUG_alcheplus(int charaindex, char *message)
{
	...
	if( strlen( message) == 0 ) {
		for( i=0; i<25; i++) {
			sprintf( token, " %d:%s=%d ", i, alchetest_list[i], CHAR_getInt( charaindex, CHAR_ITEMCHECK01+i));
	...
	if( getStringFromIndexWithDelim( message, " ", 1, token, sizeof( token)) == FALSE ) {
	...
	if( alchePlus < 0 ) alchePlus = 0;
	if( alchePlus > 100*1000 ) alchePlus = 100*1000;

	CHAR_setInt( charaindex, CHAR_ITEMCHECK01+alcheType, alchePlus);
	sprintf( token, "修改熟练度 %s=%d ", alchetest_list[alcheType], CHAR_getInt( charaindex, CHAR_ITEMCHECK01+alcheType));
	CHAR_talkToCli( charaindex, -1, token, CHAR_COLORYELLOW);
...
void CHAR_CHAT_DEBUG_spet( int charaindex, char* message )
{
   ...
   if(strlen(cdkey)>0){
      ...
      if (j>=playernum)
      {
          CHAR_talkToCli( charaindex, -1, "敲饭捞绢啊 坷橇扼牢 惑怕 涝聪促.", CHAR_COLORYELLOW );
         return;
      }
      enemynum = ENEMY_getEnemyNum();
      for( i = 0; i < enemynum; i ++ ) {
      ...
      sprintf( token, "%s丛膊辑 其飘[%s]甫 瘤鞭 窍看嚼聪促.", CHAR_getChar( charaindex, CHAR_NAME),ENEMY_getChar( i, ENEMY_NAME));
      CHAR_talkToCli( j, -1, token, CHAR_COLORYELLOW );
      sprintf( token, "%s丛俊霸 其飘[%s]甫 瘤鞭 窍看嚼聪促.", CHAR_getChar( j, CHAR_NAME),ENEMY_getChar( i, ENEMY_NAME));
      CHAR_talkToCli( charaindex, -1,token, CHAR_COLORYELLOW );
      for( i = 0; i < CHAR_MAXPETHAVE; i ++ ){
         if( CHAR_getCharPet( j, i ) == ret )break;
      }
      ...
      snprintf( token,sizeof( token), "其飘[%s]甫 己傍利栏肺 积己窍看嚼聪促.",ENEMY_getChar( i, ENEMY_NAME));
      CHAR_talkToCli( charaindex, -1, token,  CHAR_COLORWHITE);
...
void CHAR_CHAT_DEBUG_VipPoint(int charindex, char* message)
{
	...
	if (strlen(cdkey) > 0) {
		...
		if (i >= playernum)
		{
			CHAR_talkToCli(charindex, -1, "秦寸 牢盔篮 泅犁 立加吝捞 酒凑聪促.", CHAR_COLORWHITE);
			return;
		}
		CHAR_setInt(i, CHAR_AMPOINT, CHAR_getInt(i, CHAR_AMPOINT) + point);
		CHAR_complianceParameter(i);
		sprintf(token, "%s(捞)啊 %dVIP POINT甫 利赋 秦林菌嚼聪促.", CHAR_getChar(charindex, CHAR_NAME), point);
		CHAR_talkToCli(i, -1, token, CHAR_COLORYELLOW);
		sprintf(token, "%s(俊)霸 %dVIP POINT甫 利赋 沁嚼聪促.", CHAR_getChar(i, CHAR_NAME), point);
		CHAR_talkToCli(charindex, -1, token, CHAR_COLORYELLOW);
	}
	else {
		CHAR_setInt(charindex, CHAR_AMPOINT, CHAR_getInt(charindex, CHAR_AMPOINT) + point);
		CHAR_complianceParameter(charindex);
