...
//		CHAR_CharSaveLostPet( pindex, 0);
		...
		LogPet(
			...
			CHAR_getChar( pindex, CHAR_UNIQUECODE),
			pindex,
			-1
		);
...
void PET_CHECKFreePetIsIt( int petindex)
{
	...
	if( CHAR_getWorkInt( petindex, CHAR_WORKPETFOLLOWMODE) == CHAR_PETFOLLOW_NOW ) return;
...
void PET_Watchfunc( int objmeindex, int objmoveindex, CHAR_ACTION act, int x, int y, int dir,int* opt, int optlen )
{
	int		pindex;
	int		petindex;
	int		petputtime;
