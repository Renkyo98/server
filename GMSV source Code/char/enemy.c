...
BOOL ENEMYTEMP_initEnemy( char* filename )
{
	...
	char	line[256];
	int		linenum=0;//ÐÐÊýÖú¼ÇÓÃ±äÁ¿
	int		enemytemp_readlen=0;//ÓÐÐ§³èÎïÊýÁ¿±äÁ¿
	int		i,j;
	...
	for( j = 0; j < ENEMYTEMP_enemynum; j ++ ) {
		...
		sprintf(buf, "%s",sasql_getEnemybaseData(j));
		{
			...
			for( i = 0; i < E_T_DATACHARNUM; i ++ ) {
				...
				if( ret==FALSE ){
					fprint("ÆÄÀÏ ¹®¹ý ¿À·ù:[%d¹øÂ° µ¥ÀÌÅÍ %d¹øÂ° ÇÊµå]%s\n",j,i,buf);
					continue;
				}
  ...
  print( "À¯È¿ Æê ±âº»»óÅÂ °³¼ö %d...", ENEMYTEMP_enemynum );
  return TRUE;
}
...
BOOL ENEMY_initEnemy( char* filename ){
  ...
  }
	{
		for( j = 0; j < ENEMY_enemynum; j ++ ) {
			...
			if( ret==FALSE ){
				fprint("µ¥ÀÌÅÍ ¹®¹ý ¿À·ù:[%d¹øÂ° µ¥ÀÌÅÍ %d¹øÂ° ÇÊµå]%s\n",j,1,buf);
				continue;
			}
			...
			if( i == ENEMYTEMP_enemynum) {
				fprint("µ¥ÀÌÅÍ ¹®¹ý ¿À·ù: ¹øÈ£%d ÆêÀÇ ´ëÀÀ enemybase ¹øÈ£¸¦ Ã£À» ¼ö ¾øÀ½\n",enemyid);
				continue;
			}
...
#define	ENEMY_STARTINTNUM		3
	...
	print( "À¯È¿ Æê °³¼ö %d..", ENEMY_enemynum );
	return TRUE;
}
/*------------------------------------------------------------------------
...
#define	GROUP_STARTINTNUM		2
	...
	print( "À¯È¿ Á¶¿ì ±×·ì °³¼ö %d...", GROUP_groupnum );
	return TRUE;
}
/*------------------------------------------------------------------------
...
#define		PARAM_CAL( l) 	( (E_PAR(ENEMY_LV) -1)*ET_PAR( E_T_LVUPPOINT)+ ET_PAR(E_T_INITNUM)) * ET_PAR( (l))
	...
	if (level==1){//Èç¹û¹ÖÎïµÈ¼¶Îª1Ê±£¬Ìá¸ß³É³¤ÂÊ
    ...
    /* ÓÉ·Â¶ª¡õÕý±¾ÓÀÐþ */
    CharNew.data[CHAR_VITAL]    = PARAM_CAL(E_T_BASEVITAL);//Ñª³É³¤µµ
    CharNew.data[CHAR_STR]      = PARAM_CAL(E_T_BASESTR);//¹¥³É³¤µµ
    CharNew.data[CHAR_TOUGH]    = PARAM_CAL(E_T_BASETGH);//·À³É³¤µµ
    CharNew.data[CHAR_DEX]      = PARAM_CAL(E_T_BASEDEX);//Ãô³É³¤µµ
    CharNew.data[CHAR_FIREAT]   = *(tp+ E_T_FIREAT);//»ðÊôÐÔ
    CharNew.data[CHAR_WATERAT]  = *(tp+ E_T_WATERAT);//Ë®ÊôÐÔ
    CharNew.data[CHAR_EARTHAT]  = *(tp+ E_T_EARTHAT);//µØÊôÐÔ
    CharNew.data[CHAR_WINDAT]   = *(tp+ E_T_WINDAT);//·çÊôÐÔ
    CharNew.data[CHAR_MODAI]    = *(tp+ E_T_MODAI);//AIÄ£Ê½
    CharNew.data[CHAR_VARIABLEAI]    = 0;
	CharNew.data[CHAR_LV]       = level;//µÈ¼¶
	//CharNew.data[CHAR_LV]       = *(p+ ENEMY_LV);
    CharNew.data[CHAR_SLOT]     = *(tp+ E_T_SLOT);//¼¼ÄÜ¸ñ
    CharNew.data[CHAR_POISON]   = *(tp+ E_T_POISON);//¶¾¿¹
    CharNew.data[CHAR_PARALYSIS]= *(tp+ E_T_PARALYSIS);//Âé¿¹
    CharNew.data[CHAR_SLEEP]	= *(tp+ E_T_SLEEP);//Ë¯¿¹
    CharNew.data[CHAR_STONE]	= *(tp+ E_T_STONE);//Ê¯¿¹
    CharNew.data[CHAR_DRUNK]	= *(tp+ E_T_DRUNK);//×í¿¹
    CharNew.data[CHAR_CONFUSION]= *(tp+ E_T_CONFUSION);//ÂÒ¿¹
	CharNew.data[CHAR_RARE]		= *(tp+ E_T_RARE);//ÕäÏ¡¶È
	CharNew.data[CHAR_PETID]	= *(tp + E_T_TEMPNO);//±àºÅ
	CharNew.data[CHAR_CRITIAL]	= *(tp + E_T_CRITICAL);//»áÐÄÂÊ
	CharNew.data[CHAR_CRIT_RATE]	= *(tp + E_T_CRITICAL);//»áÐÄÂÊ
	CharNew.data[CHAR_COUNTER]	= *(tp + E_T_COUNTER);//·´»÷ÂÊ
	CharNew.data[CHAR_HIT_RATE]	= *(tp + E_T_COUNTER);//·´»÷ÂÊ
  CharNew.data[CHAR_LIMITLEVEL]	= *(tp + E_T_LIMITLEVEL);//¼¶ÏÞ
#ifdef _CHAR_PETDATA
...
#endif
#ifdef _PET_FUSION
	CharNew.data[CHAR_FUSIONCODE]	= *(tp + E_T_FUSIONCODE);//ÈÚºÏÂë
#endif
#ifdef _LOCK_PET_ITEM
	if(ENEMY_enemy[array].chardata[ENEMY_NAME].string[0] == '*'){
		CharNew.data[CHAR_PETLOCK] = 1;//°ó¶¨×´Ì¬
	}
#endif
...
#undef	PARAM_CAL
    ...
    strcpysafe( CharNew.string[CHAR_NAME].string,sizeof(CharNew.string[CHAR_NAME].string),(char *)ENEMYTEMP_enemy[tarray].chardata[E_T_NAME].string );
...
#ifdef _CHAR_PETDATA //
				CHAR_setInt( newindex, CHAR_PETDATAH, CHAR_getWorkInt( newindex, CHAR_WORKMAXHP));
				CHAR_setInt( newindex, CHAR_PETDATAS, CHAR_getWorkInt( newindex, CHAR_WORKATTACKPOWER));
				CHAR_setInt( newindex, CHAR_PETDATAT, CHAR_getWorkInt( newindex, CHAR_WORKDEFENCEPOWER));
				CHAR_setInt( newindex, CHAR_PETDATAD, CHAR_getWorkInt( newindex, CHAR_WORKQUICK));
#endif
			}
		}
	}
  ...
  CHAR_setWorkInt( newindex, CHAR_WORKFOXROUND, -1 ); 
	...
	CHAR_setWorkInt( newindex, CHAR_WORK_CRACKBONE2, 0 );//ÁÑ¹Ç¶Ï½î
	CHAR_setWorkInt( newindex, CHAR_WORK_POISON_DRAGNET, 0 );//¶¾ËØÖ®Íø
	CHAR_setWorkInt( newindex, CHAR_WORK_PACIFY, 0 );//¸§Î¿ÐÄÁé
	CHAR_setWorkInt( newindex, CHAR_WORK_DARK_SEAL, 0 );//?Áé·âÓ¡
#endif
#ifdef _NB_PETINITBILITY				// ¼ÇÂ¼³èÎï³õÊ¼ÄÜÁ¦Öµ±äÁ¿
		CHAR_setInt(newindex,CHAR_PETINITLV,CHAR_getInt( newindex, CHAR_LV ));
		CHAR_setInt(newindex,CHAR_PETATT1,CHAR_getWorkInt(newindex,CHAR_WORKMAXHP));
		CHAR_setInt(newindex,CHAR_PETATT2,CHAR_getWorkInt(newindex,CHAR_WORKATTACKPOWER));
		CHAR_setInt(newindex,CHAR_PETATT3,CHAR_getWorkInt(newindex,CHAR_WORKDEFENCEPOWER));
		CHAR_setInt(newindex,CHAR_PETATT4,CHAR_getWorkInt(newindex,CHAR_WORKQUICK));
#endif
    return newindex;
	...
	for( i = 0; i < ENCOUNT_GROUPMAXNUM; i ++ ){
		if( ENCOUNT_getGroupIdFromArray( array, i ) != - 1 ) {
            ...
            if( itemid != -1 ) {
            	...
            	if( j == CHAR_getMaxInventory( charaindex ) ) continue;
            }
...
BOOL PETFUSION_getIndexForChar( int toindex, int *MainIndex, int *Subindex1, int *Subindex2, char *data)
{
	...
	for( i=0; i<3; i++)	{
		...
		if( CHAR_getInt( petindex, CHAR_TRANSMIGRATION ) > 1){
			CHAR_talkToCli( toindex, -1, "2Àü»ý ÆêÀº ÇÕ¼ºÇÒ ¼ö ¾ø½À´Ï´Ù.", CHAR_COLORYELLOW);
			return FALSE;
		}
#endif
		if( rideindex == petindex) {
			CHAR_talkToCli( toindex, -1, "Å¾½Â ÁßÀÎ ÆêÀº ÇÕ¼ºÇÒ ¼ö ¾ø½À´Ï´Ù.", CHAR_COLORYELLOW);
			return FALSE;
		}
		if( i == 0 ) {
			*MainIndex = petindex;
...
#ifdef _PET_EVOLUTION
#ifdef _PET_FUSION_2
	if( (base2 = EVOLUTION_getPetTable( charaindex, petindex1, petindex2,type)) < 0 ){
#else
...
int PET_CheckIncubate( int charaindex)
{
	...
	for(i=0 ; i < CHAR_MAXPETHAVE ; i++)	{
		...
		if( (int)checkeage > (time_l+(anhour*2)) ){
			...
			sprintf( buf, "¾Ë¡´%s¡µÀÇ Ç°ÁúÀÌ ³ªºüÁ³½À´Ï´Ù.", CHAR_getUseName( petindex ));
		...
		*/
			...
			char buf[256];
			sprintf( buf, "¾Ë¡´%s¡µÀÌ(°¡) ¸ÔÀÌ ±Þ¿© °¡´É »óÅÂ°¡ µÇ¾ú½À´Ï´Ù.", CHAR_getUseName( petindex ));
			CHAR_talkToCli( charaindex, -1, buf, CHAR_COLORYELLOW);
		}
	}
	return 1;
		...
		if( (int)checkeage > (time_l+(anhour*2.5)) ){
			...
			sprintf( buf, "¾Ë¡´%s¡µÀÇ Ç°ÁúÀÌ ³ªºüÁ³½À´Ï´Ù.", CHAR_getUseName( charaindex ));
			...
			*/
			...
			char buf[256];
			sprintf( buf, "¾Ë¡´%s¡µÀÌ(°¡) ¸ÔÀÌ ±Þ¿© °¡´É »óÅÂ°¡ µÇ¾ú½À´Ï´Ù.", CHAR_getUseName( charaindex ));
			CHAR_talkToCli( masterindex, -1, buf, CHAR_COLORYELLOW);
		}
		return 1;
#endif
...
#ifdef _PET_FUSION_2 // Éè¶¨Î¹Ñø´ÎÊý
, int fusion_type
#endif
)
{
...
//		print("*CHAR_LOOPFUNCTEMP1:%s \n", "PET_CheckIncubateLoop");
		strcpysafe( ch->charfunctable[CHAR_LOOPFUNCTEMP1].string,sizeof( ch->charfunctable[CHAR_LOOPFUNCTEMP1]), "PET_CheckIncubateLoop");
		...
		CHAR_setInt(petindex,CHAR_PETATT1,CHAR_getWorkInt(petindex,CHAR_WORKMAXHP));
		CHAR_setInt(petindex,CHAR_PETATT2,CHAR_getWorkInt(petindex,CHAR_WORKATTACKPOWER));
		CHAR_setInt(petindex,CHAR_PETATT3,CHAR_getWorkInt(petindex,CHAR_WORKDEFENCEPOWER));
		CHAR_setInt(petindex,CHAR_PETATT4,CHAR_getWorkInt(petindex,CHAR_WORKQUICK));
#endif
	if( CHAR_CHECKINDEX( toindex) ){
		for(i = 0; i < CHAR_MAXPETHAVE; i++){
			char msgbuf[256];
...
BOOL PETFUSION_AddEgg( int toindex, int petID, int PetCode)
{
	...
	if( i == CHAR_MAXPETHAVE )      {
		snprintf( msgbuf,sizeof( msgbuf), "Æê º¸À¯ ÇÑµµ ÃÊ°ú£¡£¡");
	...
	snprintf( msgbuf,sizeof( msgbuf), "%s À»(¸¦) È¹µæÇß½À´Ï´Ù.", CHAR_getChar(petindex2,CHAR_NAME));
	CHAR_talkToCli( toindex, -1, msgbuf,  CHAR_COLORYELLOW);
