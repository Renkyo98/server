...
static int getInt(lua_State *L) 
{
	...
	const int element = getCharBaseValue(L, 2, PetSkillBaseInt, arraysizeof(PetSkillBaseInt),1);
...
{
	const int index = luaL_checkint(L, 1);
	const int element = getCharBaseValue(L, 2, PetSkillBaseChar, arraysizeof(PetSkillBaseChar),1);
