...
static int getInt(lua_State *L) 
{
  ...
  const int element = getCharBaseValue(L, 2, BattleBaseInt, arraysizeof(BattleBaseInt),1);
  int ret = -1;
...
static int setLUAFunctionPointer(lua_State *L) 
{
	size_t l;
	const int battleindex = luaL_checkint(L, 1);
	const int functype = getCharBaseValue(L, 2, BattleBaseEvent, arraysizeof(BattleBaseEvent),1);
