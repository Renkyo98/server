...
static BOOL BATTLE_DuckCheck(int attackindex,int defindex)
{
	...
	gDuckPer = 0;
	if(CHAR_getWorkInt( attackindex, CHAR_WORKBATTLECOM1) == BATTLE_COM_S_CHARGE_OK ){//技能状态判断
		return FALSE;
	}
	if( CHAR_getWorkInt( defindex, CHAR_WORKBATTLECOM1 ) == BATTLE_COM_GUARD ){//技能状态判断
		return FALSE;
	}
	...
	if( CHAR_getInt( defindex, CHAR_WHICHTYPE ) == CHAR_TYPEPLAYER ){//如果防方对像是玩家
		Df_Luck = CHAR_getWorkInt( defindex, CHAR_WORKFIXLUCK );//取幸运值
	}
	if( CHAR_getInt( attackindex, CHAR_WHICHTYPE ) == CHAR_TYPEENEMY
	&&  CHAR_getInt( defindex, CHAR_WHICHTYPE ) == CHAR_TYPEPET	){//如果攻方是怪物，防方是宠物，攻方敏捷*0.8
		At_Dex *= 0.8;
	}else
	if( CHAR_getInt( attackindex, CHAR_WHICHTYPE ) != CHAR_TYPEENEMY
	&&  CHAR_getInt( defindex, CHAR_WHICHTYPE ) == CHAR_TYPEPET//如果攻方不是怪物，防方是宠物，防方敏捷*0.8
	){
		Df_Dex *= 0.8;
	}else
	if( CHAR_getInt( attackindex, CHAR_WHICHTYPE ) != CHAR_TYPEPLAYER
	&&  CHAR_getInt( defindex, CHAR_WHICHTYPE ) == CHAR_TYPEPLAYER	){//如果攻方不是玩家，防方不是玩家，攻方敏捷*0.6
		At_Dex *= 0.6;
	}else
	if( CHAR_getInt( attackindex, CHAR_WHICHTYPE ) == CHAR_TYPEPLAYER
	&&  CHAR_getInt( defindex, CHAR_WHICHTYPE ) != CHAR_TYPEPLAYER	){//如果攻方是玩家，防方不是玩家，防御方敏捷*0.6
		Df_Dex *= 0.6;
	}
	...
	per *= wari;//回避 * 修正值1
	per += Df_Luck;//回避 + 运气
	per += CHAR_getInt(defindex, CHAR_AVOID_RATE) - CHAR_getInt(attackindex, CHAR_HIT_RATE);//回避 + 运气
	...
	if( CHAR_getWorkInt( defindex, CHAR_WORKBATTLECOM1 ) == BATTLE_COM_S_NOGUARD ){//如果防方用不防守战法
		per += CHAR_GETWORKINT_HIGH( defindex, CHAR_WORKBATTLECOM3 );//回避+技能的回避值
	}
	...
	per *= 100;//回避 * 100
	// 防止溢出
	if( per > KAWASHI_MAX_RATE*100 ) per = KAWASHI_MAX_RATE*100; //如果回避大于 7500则直接等于7500
	if( per <= 0 ) per = 1;
	...
	if( CHAR_getInt( attackindex, CHAR_WHICHTYPE ) == CHAR_TYPEPLAYER ){
		...
		per -= RAND( AddHit*0.8, AddHit*1.2);//回避率 - (装备命中*0.8 ~ 装备命中*1.2之间的随机值)
...
#endif
#ifdef _PROFESSION_ADDSKILL
	if( CHAR_getWorkInt( attackindex, CHAR_WORKBATTLECOM1 ) == BATTLE_COM_S_CHAOS ){ //攻房混乱攻击时防方回避增加
		per += (per*0.4);	//回避 * 0.4
	}
#endif

	if( RAND( 1, 10000 ) <= per ){//取1~10000的随机数，如果随机数小于或等于回避率，攻方命中成功，防方回避失败
		flg = TRUE;
#ifdef _PROFESSION_SKILL			// WON ADD 人物职业技能
		// 回避技能升级
...
static int BATTLE_CriticalCheckPlayer( int attackindex, int defindex )//会心一击的计算
{
	...
	At_Dex = CHAR_getWorkInt( attackindex, CHAR_WORKFIXDEX );//得到发起攻击对象的敏捷
	Df_Dex = CHAR_getWorkInt( defindex, CHAR_WORKFIXDEX );//得到受到攻击对象的敏捷

	if( CHAR_getInt( attackindex, CHAR_WHICHTYPE ) == CHAR_TYPEPLAYER ){//如果攻击对象为玩家
	...
	&&  CHAR_getInt( defindex, CHAR_WHICHTYPE ) == CHAR_TYPEENEMY){//如果攻击对象为宠物，受到对象为怪物
		Df_Dex *= 0.8;//受到对象敏捷*0.8
	}else
	if( CHAR_getInt( attackindex, CHAR_WHICHTYPE ) == CHAR_TYPEENEMY
	&&  CHAR_getInt( defindex, CHAR_WHICHTYPE ) == CHAR_TYPEPET){//如果攻击对象为怪物，受到对象为宠物
		divpara = 10.0;//修正值1
		root = 0;//标识
	}else
	if( CHAR_getInt( attackindex, CHAR_WHICHTYPE ) != CHAR_TYPEPLAYER
	&&  CHAR_getInt( defindex, CHAR_WHICHTYPE ) == CHAR_TYPEPLAYER){//如果攻击对象不是玩家，受到对象为玩家
		divpara = 10.0;//修正值1
		root = 0;//标识
	}else
		if( CHAR_getInt( attackindex, CHAR_WHICHTYPE ) == CHAR_TYPEPLAYER
		&&  CHAR_getInt( defindex, CHAR_WHICHTYPE ) != CHAR_TYPEPLAYER){//如果攻击对象为玩家，受到对象不是玩家
	...
	if( Work <= 0 ) Work = 0;
	if( root == 1 ){//如果标识为1时进行 平方根计算Work 之后加 (武器的会心率 * 0.5)
	...
	per *= 100;//会心率 * 100
	if( per < 0 ) per = 1;//防溢出设置
	if( per > 10000 ) per = 10000;//防溢出设置
#ifdef _PETSKILL_LER
	// 雷尔不会出现会心
...
// end
	...
	|| GETTHIRDVALUE( CHAR_getWorkInt( GuardianIndex, CHAR_WORK_PROVOKEFIGHT ) ) > 0 //挑畔反击
#endif
#ifdef _PRO_ABSOLUTE_DEFENSE
...
#endif
		{
			...
			if(CHAR_getWorkInt( defindex , CHAR_WORKTACTICS) == TACTICS_BOSS){
				...
				if( attackNo >= SIDE_OFFSET ){
					attackSide = 1; SubNo = attackNo - SIDE_OFFSET;
				}else{
					attackSide = 0; SubNo = attackNo;
				}
...
// 雷尔不能被打飞
		...
		if (react != BATTLE_MD_ACUPUNCTURE) flg |= BCF_DEATH;
#else
		flg |= BCF_DEATH;
#endif
		if( ultimate == 1 ){
	...
	if(gBattleStausSuccess > 0 && RAND( 1, 100 ) < gBattleStausSuccess)
		successFlg = 1;
	//确认有无其他状态
	if( gBattleStausChange >= BATTLE_ST_END || gBattleStausChange <= 0 ) successFlg = 0;
	for( i = 1; i < BATTLE_ST_END; i++ ){//若已有异常状态则return
		if(StatusTbl[i] < 0) continue;
	...
	if ((damage > 0) && (CHAR_getWorkInt(attackindex, CHAR_WORKBATTLECOM1) == BATTLE_COM_S_ATTCRAZED)){
		int array=-1,temp2;
		...
		if( ( pszOption = strstr( temp, "回%" ) ) ){
			...
			if( CHAR_getWorkInt( defindex, CHAR_MYSKILLDUCK) > 0 ){
				...
				CHAR_setWorkInt( defindex, CHAR_MYSKILLDUCKPOWER, ducktemp+(ducktemp*(temp2/100.0)) );
			}
		}
	}
#endif
...
BOOL BATTLE_CaptureItemCheck( int attackindex, int defindex )
{
	...
	for( i=0; i<MAXCAPTRUEFREE; i++ ){
		...
		//没找到
		if( j>= CHAR_getMaxInventory( attackindex ) ) return FALSE;
	}
	return TRUE;
#else
	...
	if( flg == 1 ){
		...
		if( pindex == -1 ){
			...
			LogPet(
				...
				CHAR_getInt( attackindex, CHAR_X ),
				CHAR_getInt( attackindex, CHAR_Y ),
				CHAR_getChar( defindex, CHAR_UNIQUECODE),
				defindex,
				attackindex
			);
			{
				int array = ENCOUNT_getEncountAreaArray(CHAR_getInt(attackindex, CHAR_FLOOR), CHAR_getInt(attackindex, CHAR_X), CHAR_getInt(attackindex, CHAR_Y));
				if (array != -1)
				{
		 ...
		 CaptureOkFunction( attackindex, defindex );
			...
			if( needItemId >= 0 ){
				for( i = CHAR_STARTITEMARRAY; i < CHAR_getMaxInventory( attackindex ); i ++ ) {
...
)
{
	...
	if( CHAR_getWorkInt( defindex, CHAR_WORKTACTICS) == TACTICS_BOSS ) return FALSE;
#endif
#ifdef _PRO_ABSOLUTE_DEFENSE
	if( CHAR_getWorkInt( defindex, CHAR_WORK_ABSOLUTE) > 0 ) return FALSE;
#endif
...
//	print(" nono宠改休息 ");
	...
	CHAR_AddCharm( oyaindex, CH_FIX_PETESCAPE );
			...
			LogPet(
				...
				CHAR_getChar( attackindex, CHAR_UNIQUECODE),
				attackindex,
				oyaindex
			);
...
void BATTLE_Steal( int battleindex, int attackNo, int defNo )
{
	...
	if( RAND( 1, 100 ) < per ){
		...
		if( RAND( 1, 100 ) < 50 ){
			...
			stealmode = 1;
			for( j = 0, i = CHAR_STARTITEMARRAY; i < CHAR_getMaxInventory( defindex ); i ++ ){
...
int BATTLE_S_FallGround( int battleindex, int attackNo, int defNo, int skill_type)
{
	...
	//取得攻击後状态
	iWork = BATTLE_AttackSeq( attackindex, defindex, &damage,&Guardian, BATTLE_COM_S_FALLRIDE );
	...
	if( skill_type == BATTLE_COM_S_EXPLODE && damage > 0 && react == 0
		...
		if( 
			...
			if( CHAR_getInt( defindex , CHAR_WHICHTYPE ) == CHAR_TYPEPLAYER )	{
				if( CHAR_getInt( defindex, CHAR_RIDEPET) >= 0 ) {
#ifdef _PETSKILL_RIDE
						CHAR_setWorkInt(defindex, CHAR_WORK_RIDE, MAKE2VALUE(CHAR_getInt(defindex, CHAR_RIDEPET), GETLOWVALUE(CHAR_getWorkInt(defindex, CHAR_WORK_RIDE))));
#endif
						flg |= BCF_FALL;
						CHAR_setInt( defindex, CHAR_RIDEPET, -1 );
						BATTLE_changeRideImage( defindex );
...
int BATTLE_S_AttackDamage( int battleindex, int attackNo, int defNo, int skill_type, int skill)
{
	...
	switch( skill_type)	{
	case -1:
		snprintf( szCommand, sizeof( szCommand ), "BH|a%X|r%X|f%X|d%X|p%X|FF|",attackNo, defNo, flg, damage, petdamage );
		BATTLESTR_ADD( szCommand );
		break;
	...
	case BATTLE_COM_S_DAMAGETOHP:
		...
		snprintf( szCommand, sizeof( szCommand ), "Bh|a%X|r%X|f%X|d%X|p%X|j%X|FF|",attackNo, defNo, flg, damage, petdamage, otherdamage);
		BATTLESTR_ADD( szCommand );
		break;
#endif
	...
	case BATTLE_COM_S_MPDAMAGE:
		...
		snprintf( szCommand, sizeof( szCommand ), "Bp|a%X|r%X|f%X|d%X|p%X|j%X|FF|",attackNo, defNo, flg, damage, petdamage, otherdamage);
		BATTLESTR_ADD( szCommand );
		break;
#endif
	...
	case BATTLE_COM_S_MDFYATTACK:
		flg |= BCF_MODIFY;
		snprintf( szCommand, sizeof( szCommand ), "BH|a%X|r%X|f%X|d%X|p%X|g%X|FF|",attackNo, defNo, flg, damage, petdamage, skill);
		BATTLESTR_ADD( szCommand );
		break;
#endif
#ifdef _PSKILL_MODIFY
	...
	case BATTLE_COM_S_2TIMID:
		...
		case BATTLE_COM_S_ANTINTER:
		{
			...
			snprintf( szCommand, sizeof( szCommand ), "BH|a%X|r%X|f%X|d%X|p%X|FF|",attackNo, defNo, flg |= BCF_ANTINTER , 0, 0 );
			BATTLESTR_ADD( szCommand );
			if( CHAR_getInt( defindex, CHAR_WHICHTYPE ) == CHAR_TYPEPET ){
				int defkoyaindex = CHAR_getWorkInt( defindex, CHAR_WORKPLAYERINDEX );
...
int battle_profession_attack_fun(int battleindex, int attackNo, int defNo, int charaindex)
{
	...
	int damage=0, petdamage=0, i=0, iWork, Guardian=-1,react, otherdamage=0;//额外伤害
	int flg = 0, DefSide=0,ultimate=0;
	BOOL iRet = FALSE;
...
#ifdef _PRO3_ADDSKILL
	if( GETHIGHVALUE(CHAR_getWorkInt( charaindex, CHAR_WORK_DARK_SEAL ) ) > 0 //?灵封印
		&& CHAR_getInt( charaindex, PROFESSION_CLASS ) == 2 ){
		sprintf( szCommand, "BD|r%X|0|C|%X|", attackNo, PROFESSION_SKILL_getskillArray( skill ) );
		BATTLESTR_ADD( szCommand );
		return iRet;
	}
#endif
...
#ifdef _PRO3_ADDSKILL
	if( (pszP = strstr( pszOption, "冷%" ) ) != NULL )	sscanf( pszP+3, "%d", &cooltime );
#endif
	// 准备期的动画
		...
		case BATTLE_COM_S_CRACK_BONE:		//裂骨断筋
			{
				if( GETHIGHVALUE(CHAR_getWorkInt( charaindex, CHAR_WORK_CRACKBONE2 )) > 0  ){				
					sprintf(szBuffer,"裂骨断筋于 %d 回合后才可使用。",GETHIGHVALUE(CHAR_getWorkInt( charaindex, CHAR_WORK_CRACKBONE2 ))); 
					CHAR_talkToCli( charaindex, -1, szBuffer, CHAR_COLORYELLOW);
					return iRet;
				}
				if( (pszP = strstr( pszOption, "DAM" ) ) != NULL )	sscanf( pszP+3, "%d", &otherdamage );
				break;
			}
#endif
		default:
...
//			skill_type=-1;
	...
	if( skill_type == BATTLE_COM_S_CRACK_BONE && damage > 0 ){//裂骨断筋
		...
		CHAR_setWorkInt( attackindex, CHAR_WORK_CRACKBONE2, MAKE2VALUE(cooltime,ltemp) );
	}
#endif
...
*/
	...
	//	(attackNo >= 10)? CHAR_COLORGRAY : CHAR_COLORPURPLE ) ;
#ifdef _PRO3_ADDSKILL
	if( skill_type == BATTLE_COM_S_CRACK_BONE && CHAR_getInt( defindex, CHAR_HP ) > 0 && iDamage > 0 ){//裂骨断筋
		int str=0, turn=0,per=0;
		int htemp,ltemp;
		if( (pszP = strstr( pszOption, "STR" ) ) != NULL )	sscanf( pszP+3, "%d", &str );
		if( (pszP = strstr( pszOption, "回%" ) ) != NULL )	sscanf( pszP+3, "%d", &turn );
		if( (pszP = strstr( pszOption, "避%" ) ) != NULL )	sscanf( pszP+3, "%d", &per );
...
int battle_profession_attack_magic_fun(int battleindex, int attackNo, int defNo, int charaindex)
{
	...
	if( GETHIGHVALUE(CHAR_getWorkInt( charaindex, CHAR_WORK_DARK_SEAL ) ) > 0 //?灵封印
		&& CHAR_getInt( charaindex, PROFESSION_CLASS ) == 2 ){
		char szBuffer[64]="";
		sprintf( szBuffer, "BD|r%X|0|C|%X|", attackNo, PROFESSION_SKILL_getskillArray( skill ) );
		BATTLESTR_ADD( szBuffer );
		return FALSE;
	}
#endif
...
#ifdef BOSS_AI
			if( BattleArray[battleindex].Side[CHAR_getWorkInt( charaindex,CHAR_WORKBATTLESIDE)^1].type == BATTLE_S_TYPE_ENEMY ){
				int isearch , bossindex , npcside ;
				npcside = CHAR_getWorkInt( charaindex,CHAR_WORKBATTLESIDE)^1 ;
				for( isearch = 0 ; isearch < BATTLE_ENTRY_MAX ; isearch++ ){ //找看看有没有boss
					bossindex = BattleArray[battleindex].Side[npcside].Entry[isearch].charaindex ;
					if(CHAR_getWorkInt( bossindex , CHAR_WORKTACTICS) == TACTICS_BOSS){
						print("\n boss ai 使用移行换位");
						BattleArray[battleindex].Side[npcside^1].Entry[attackNo].proskilluse = 21;
					}
				}
			}
#endif
...
#else
					if( skill_level >= 10 )			avoid = 60;
					else if( skill_level >= 9 )		avoid = 50;
					else if( skill_level >= 8 )		avoid = 45;
					else if( skill_level >= 6 )		avoid = 40;
					else if( skill_level >= 5 )		avoid = 35;
					else if( skill_level >= 3 )		avoid = 30;
					else							avoid = 25;
					
					if( skill_level >= 10 )		turn = 4;
					else if( skill_level >= 6 ) turn = 3;
					else						turn = 2;
#endif					
...
#ifdef _PRO_ABSOLUTE_DEFENSE
	if( (pszP = strstr( pszOption, "冷%" ) ) != NULL )	sscanf( pszP+3, "%d", &cooltime );
	if( (pszP = strstr( pszOption, "道" ) ) != NULL )	sscanf( pszP+2, "%d", &needitem );//消耗道具
#endif
	// 目标地球一周，不动作
	if( defNo >= 0 && defNo <= 19 ){
		defindex = BATTLE_No2Index( battleindex, defNo );
		...
		case BATTLE_COM_S_MEDICATION://制药学
			...
			if( skill_type == BATTLE_COM_S_ABSOLUTE_DEFENSE ){
				...
				if( GETHIGHVALUE(CHAR_getWorkInt( toindex, CHAR_WORK_ABSOLUTE2 )) > 0 ){
					sprintf(szCommand,"绝对防御于 %d 回合后才可使用。",GETHIGHVALUE(CHAR_getWorkInt( toindex, CHAR_WORK_ABSOLUTE2 )) );
			...
			if( skill_type == BATTLE_COM_S_PACIFY ){//抚慰心灵
				if( GETFIRSTVALUE(CHAR_getWorkInt( charaindex, CHAR_WORK_PACIFY )) > 0 ){
					sprintf(szCommand,"抚慰心灵于 %d 回合后才可使用。",GETFIRSTVALUE(CHAR_getWorkInt( charaindex, CHAR_WORK_PACIFY )) );
					CHAR_talkToCli( charaindex, -1, szCommand, CHAR_COLORYELLOW );
					return iRet;
				}
			}
				...
				if( GETLOWVALUE(CHAR_getWorkInt( charaindex, CHAR_WORK_DARK_SEAL )) > 0 ){
					sprintf(szCommand,"?灵封印于 %d 回合后才可使用。",GETLOWVALUE(CHAR_getWorkInt( charaindex, CHAR_WORK_DARK_SEAL )) );
					CHAR_talkToCli( charaindex, -1, szCommand, CHAR_COLORYELLOW );
					return iRet;
				}
			}
			else if( skill_type == BATTLE_COM_S_DEMON_SPEECH ){//恶魔诅咒
				if( GETTHIRDVALUE(CHAR_getWorkInt( charaindex, CHAR_WORK_DEMON_SPEECH )) > 0 ){
					sprintf(szCommand,"恶魔诅咒于 %d 回合后才可使用。",GETTHIRDVALUE(CHAR_getWorkInt( charaindex, CHAR_WORK_DEMON_SPEECH )) );
					CHAR_talkToCli( charaindex, -1, szCommand, CHAR_COLORYELLOW );
					return iRet;
				}
			}
				...
				if( CHAR_getWorkInt( charaindex, CHAR_WORK_FIRST_AID ) > 0 ){
					sprintf(szCommand,"战场急救于 %d 回合后才可使用。", CHAR_getWorkInt( charaindex, CHAR_WORK_FIRST_AID ) );
					CHAR_talkToCli( charaindex, -1, szCommand, CHAR_COLORYELLOW );
					return iRet;
				}
			}
				...
				if( GETHIGHVALUE(CHAR_getWorkInt( charaindex, CHAR_WORK_ROAR )) > 0 ){
					sprintf(szCommand,"战狼怒吼于 %d 回合后才可使用。", GETHIGHVALUE(CHAR_getWorkInt( charaindex, CHAR_WORK_ROAR )) );
					CHAR_talkToCli( charaindex, -1, szCommand, CHAR_COLORYELLOW );
					return iRet;
				}
			}
				...
				if( CHAR_getWorkInt( charaindex, CHAR_WORK_MEDICATION ) > 0 ){
					sprintf(szCommand,"制药学于 %d 回合后才可使用。", CHAR_getWorkInt( charaindex, CHAR_WORK_MEDICATION ) );
					CHAR_talkToCli( charaindex, -1, szCommand, CHAR_COLORYELLOW );
					return iRet;
				}
			}
			...
			if( skill_type == BATTLE_COM_S_KILLME )
				...
				if( BattleArray[battleindex].Side[CHAR_getWorkInt( charaindex,CHAR_WORKBATTLESIDE)^1].type == BATTLE_S_TYPE_ENEMY ){
					...
					for( isearch = 0 ; isearch < BATTLE_ENTRY_MAX ; isearch++ ){ //找看看有没有boss
						bossindex = BattleArray[battleindex].Side[npcside].Entry[isearch].charaindex ;
						if( !CHAR_CHECKINDEX( bossindex ) )continue;
						if(CHAR_getWorkInt( bossindex , CHAR_WORKTACTICS) == TACTICS_BOSS){
							print("\n boss ai 使用天罗地网");
							BattleArray[battleindex].Side[npcside^1].Entry[attackNo].proskilluse = 48;
						}
					}
				}
#endif
...
#ifdef _PRO3_ADDSKILL
				if( skill_type == BATTLE_COM_S_DRAGNET && GETTHIRDVALUE(CHAR_getWorkInt( toindex, CHAR_WORK_POISON_DRAGNET ) ) > 0 ){
					;;
				}
				else if( skill_type == BATTLE_COM_S_POISON_DRAGNET && (rand()%100 < Success) //毒素之网
					&& CHAR_getWorkInt( toindex, CHAR_WORKDRAGNET ) <= 0 ){
					if( GETTHIRDVALUE(CHAR_getWorkInt( toindex, CHAR_WORK_POISON_DRAGNET ) ) == 0 ){
						int poisontemp=10;
						if( (pszP = strstr( pszOption, "素%" ) ) != NULL )	sscanf( pszP+3, "%d", &poisontemp );
						sprintf( szCommand, "BD|r%X|0|A|%X|", ToList[i], 12 );
						BATTLESTR_ADD( szCommand );
						CHAR_setWorkInt( toindex, CHAR_WORK_POISON_DRAGNET, MAKE2VALUE(turn+1,poisontemp) );
					...
					if( (pszP = strstr( pszOption, "忠%" ) ) != NULL )	sscanf( pszP+3, "%d", &pacifytemp2 );
					sprintf( szCommand, "BD|r%X|0|B|%X|", ToList[i], pacifytemp2 );
					BATTLESTR_ADD( szCommand );
					//if( pacifyl2 == 0 )
						CHAR_setWorkInt( toindex, CHAR_WORK_PACIFY, MAKE4VALUE(pacifyl1,turn+1,pacifytemp2,pacifyr2) );
					//else
					//	CHAR_setWorkInt( toindex, CHAR_WORK_PACIFY, MAKE4VALUE(pacifyl1,pacifyl2,pacifyr1+pacifytemp2,pacifyr2) );
					...
					CHAR_setWorkInt( charaindex, CHAR_WORK_PACIFY, MAKE4VALUE(pacifyl1,pacifyl2,pacifyr1,cooltime) );
				}else if( skill_type == BATTLE_COM_S_DARK_SEAL ){//?灵封印
					...
					CHAR_setWorkInt( toindex, CHAR_WORK_DARK_SEAL, MAKE2VALUE(turn+1,darkseal) );
					darksealtemp = CHAR_getWorkInt( charaindex, CHAR_WORK_DARK_SEAL );
					darkseal = GETHIGHVALUE(darksealtemp);
					CHAR_setWorkInt( charaindex, CHAR_WORK_DARK_SEAL, MAKE2VALUE(darkseal,cooltime) );
					...
					if( strstr( pszOption, "darkseal" ) ){//斗气导引
						...
						if( (pszP = strstr( pszOption, "率%" ) ) != NULL )	sscanf( pszP+3, "%d", &darkseal );
						...
						CHAR_setWorkInt( toindex, CHAR_WORK_DARK_SEAL, MAKE2VALUE(0,darkseal) );
						...
						//int h1=GETFOURTHVALUE(CHAR_getWorkInt( toindex, CHAR_WORK_DEMON_SPEECH ));
						int h2=GETTHIRDVALUE(CHAR_getWorkInt( toindex, CHAR_WORK_DEMON_SPEECH ));
						//int l1=GETSECONDVALUE(CHAR_getWorkInt( toindex, CHAR_WORK_DEMON_SPEECH ));
						//int l2=GETFIRSTVALUE(CHAR_getWorkInt( toindex, CHAR_WORK_DEMON_SPEECH ));
						if( (pszP = strstr( pszOption, "率%" ) ) != NULL )	sscanf( pszP+3, "%d", &dtemp );
						if( rand()%100 > dtemp ){
							CHAR_talkToCli( charaindex, -1, "神灵之赐失败!", CHAR_COLORYELLOW );
							return iRet;
						}
						CHAR_setWorkInt( toindex, CHAR_WORK_DEMON_SPEECH, MAKE4VALUE(0,h2,0,0) );
						...
						int h1=GETFOURTHVALUE(CHAR_getWorkInt( charaindex, CHAR_WORK_DEMON_SPEECH ));
						int h2=GETTHIRDVALUE(CHAR_getWorkInt( charaindex, CHAR_WORK_DEMON_SPEECH ));
						int l1=GETSECONDVALUE(CHAR_getWorkInt( charaindex, CHAR_WORK_DEMON_SPEECH ));
						int l2=GETFIRSTVALUE(CHAR_getWorkInt( charaindex, CHAR_WORK_DEMON_SPEECH ));
						if( Absolute_Defense_Check(toindex) == TRUE )
							return iRet;
						CHAR_setWorkInt( charaindex, CHAR_WORK_DEMON_SPEECH, MAKE4VALUE(h1,cooltime,l1,l2) );
						
						if( (pszP = strstr( pszOption, "HP%" ) ) != NULL )	sscanf( pszP+3, "%d", &l1 );
						if( (pszP = strstr( pszOption, "MP%" ) ) != NULL )	sscanf( pszP+3, "%d", &l2 );
						...
						CHAR_setWorkInt( toindex, CHAR_WORK_DEMON_SPEECH, MAKE4VALUE(turn+1,h2,l1,l2) );
					...
					if( CHAR_getInt( toindex, CHAR_HP ) <= 0 )
						return iRet;
					if( (pszP = strstr( pszOption, "血%" ) ) != NULL )	sscanf( pszP+3, "%d", &addhp );
					addhppet = addhp;
					...
					if( ridepet == -1 ){
						...
						CHAR_setInt( toindex, CHAR_HP, 
							min( addhp+CHAR_getInt( toindex, CHAR_HP ), CHAR_getWorkInt( toindex, CHAR_WORKMAXHP ) ) );
						addhppet = CHAR_getWorkInt( ridepet, CHAR_WORKMAXHP )*(addhppet*0.01);
						CHAR_setInt( ridepet, CHAR_HP, 
							min( addhppet+CHAR_getInt( ridepet, CHAR_HP ), CHAR_getWorkInt( ridepet, CHAR_WORKMAXHP ) ) );
					}
					CHAR_setWorkInt( charaindex, CHAR_WORK_FIRST_AID, cooltime );
					snprintf( szCommand, sizeof(szCommand),"BD|r%X|%X|%X|d%X|p%X|", ToList[i], BD_KIND_HP, 1, addhp, ridepet?addhppet:0 );
					BATTLESTR_ADD( szCommand );
				}else if( skill_type == BATTLE_COM_S_PROAR ){//战狼怒吼
					int delhp,delmp;
					...
					int ridepet = BATTLE_getRidePet( toindex );
					if( (pszP = strstr( pszOption, "HP%" ) ) != NULL )	sscanf( pszP+3, "%d", &delhp );
					if( (pszP = strstr( pszOption, "MP%" ) ) != NULL )	sscanf( pszP+3, "%d", &delmp );
					//减 MP
					if( CHAR_getInt( toindex, CHAR_WHICHTYPE ) == CHAR_TYPEPLAYER ){
					...
					BATTLESTR_ADD( szCommand );
					delhp = 0;
					if( (pszP = strstr( pszOption, "率%" ) ) != NULL )	sscanf( pszP+3, "%d", &delhp );
					if( rand()%100 < delhp && old_hp > 0 ){
						if( (pszP = strstr( pszOption, "惊%" ) ) != NULL )	sscanf( pszP+3, "%d", &delmp );
					...
					CHAR_setWorkInt( charaindex, CHAR_WORK_ROAR, MAKE2VALUE(cooltime,delhp) );
				}else if( skill_type == BATTLE_COM_S_MEDICATION ){//制药学
					int eitemno;
					if( (pszP = strstr( pszOption, "药%" ) ) != NULL )	sscanf( pszP+3, "%d", &eitemno );
					for( j = CHAR_STARTITEMARRAY ; j < CHAR_getMaxInventory( charaindex ) ; j++ ){
						itemindex = CHAR_getItemIndex(charaindex,j);
						if( itemindex == -1 )
							break;
						else if( eitemno == ITEM_getInt( itemindex, ITEM_ID) ){
							CHAR_talkToCli( charaindex, -1, "有相同的药品存在!", CHAR_COLORYELLOW);
							return iRet;
						}
					}
                ...
                CHAR_setWorkInt( toindex, StatusTbl[BATTLE_ST_RESIST_T], turn + 1 );
						...
						if( StatusTbl[status] == CHAR_WORKANGER )
							...
							if( StatusTbl[status] == CHAR_WORK_ABSOLUTE ){
								old_dex = 300;
								if( (pszP = strstr( pszOption, "减%" ) ) != NULL )	sscanf( pszP+3, "%d", &old_dex );
								CHAR_setWorkInt( toindex, CHAR_WORK_ABSOLUTE2, MAKE2VALUE(cooltime,old_dex) );
							}
#endif
						}
						if( status == BATTLE_ST_PARALYSIS
						...
						if( skill_type == BATTLE_COM_S_OBLIVION ){
							...
							if (skill_level >= 10)          num = RAND(6, 7);
							else if (skill_level >= 8)      num = RAND(4, 6);
							else if (skill_level >= 6)		num = RAND(3, 5);
							else if (skill_level >= 4)		num = RAND(2, 4);
							else if (skill_level >= 1)		num = RAND(1, 3);
		...
		case BATTLE_COM_S_SHIELD_ATTACK:	// 盾击
			{
				...
				if( BattleArray[battleindex].Side[DefSide].type == BATTLE_S_TYPE_ENEMY ){
					...
					for( isearch = 0 ; isearch < BATTLE_ENTRY_MAX ; isearch++ ){ //找看看有没有boss
						bossindex = BattleArray[battleindex].Side[DefSide].Entry[isearch].charaindex ;
						if(CHAR_getWorkInt( bossindex , CHAR_WORKTACTICS) == TACTICS_BOSS){
							print("\n boss ai 使用盾击");
							BattleArray[battleindex].Side[DefSide^1].Entry[attackNo].proskilluse = 21;
						}
					}
				}
#endif
	...
	int iType)
{
	...
	if( CHAR_getWorkInt( iDefindex, CHAR_WORKBATTLECOM1 ) == BATTLE_COM_S_EARTHROUND0 ){
...
{ 
  ...
  BATTLE_MultiParamChangeTurn( battleindex, attackNo, defNo, status, MAGIC_EFFECT_USER, ReceveEffect, turn, Success);
