		...
		if( BattleArray[battleindex].Side[CHAR_getWorkInt( charaindex,CHAR_WORKBATTLESIDE)^1].type == BATTLE_S_TYPE_ENEMY ){
			...
			for( isearch = 0 ; isearch < BATTLE_ENTRY_MAX ; isearch++ ){ //找看看有没有boss
				...
				if(CHAR_getWorkInt( bossindex , CHAR_WORKTACTICS) == TACTICS_BOSS){
					//print("\n boss ai 使用加血精灵 ");
					BattleArray[battleindex].Side[npcside^1].Entry[attackNo].addhpskill += 1;
				}
			}
		}
	}
...
#ifdef _FIX_MAGICDAMAGE
void BATTLE_MultiAttMagic( int battleindex, int attackNo, int toNo,int attIdx ,int FieldAttr ,int Power, int MagicLv)
#else
...
#ifdef BOSS_AI
				if( !DefIsPlayer && CHAR_getWorkInt( charaidx , CHAR_WORKTACTICS) == TACTICS_BOSS ){
					char buff[256];
					float  adjustvalue;
					if( NPC_Util_GetStrFromStrWithDelim( CHAR_getWorkChar( charaidx, CHAR_WORKBATTLE_TACTICSOPTION),"A2" , buff, sizeof( buff)) != NULL ){
...
//***********************************************************
{
	...
	for( i = 0; ToList[i] != -1; i ++ ){
		...
		if( tostatus == 0 ){//正常
			...
			if( GETHIGHVALUE(CHAR_getWorkInt( toindex, CHAR_WORK_POISON_DRAGNET ) ) > 0 ){//毒素之网
				int ltemp=GETLOWVALUE(CHAR_getWorkInt( toindex, CHAR_WORK_POISON_DRAGNET ));
				CHAR_setWorkInt( toindex, CHAR_WORK_POISON_DRAGNET, MAKE2VALUE(0,ltemp) );
...
)
{
	...
	battleindex = CHAR_getWorkInt( charaindex, CHAR_WORKBATTLEINDEX );//获取战场索引
	attackNo = BATTLE_Index2No( battleindex, charaindex );
	...
	if( toNo != attackNo ){
		if( BattleArray[battleindex].Side[CHAR_getWorkInt( charaindex,CHAR_WORKBATTLESIDE)^1].type == BATTLE_S_TYPE_ENEMY ){
			...
			for( isearch = 0 ; isearch < BATTLE_ENTRY_MAX ; isearch++ ){ //找看看有没有boss
				...
				if(CHAR_getWorkInt( bossindex , CHAR_WORKTACTICS) == TACTICS_BOSS){
					//print("\n boss ai 使用净化精灵");
					BattleArray[battleindex].Side[npcside^1].Entry[attackNo].purgeflg += 1;
				}
			}
		}
	}
...
// end
	...
	if( command == BATTLE_COM_S_ICE_ARROW ){
		...
		pszOption = PROFESSION_SKILL_getChar( skill, PROFESSION_SKILL_OPTION);// 取得技能参数
		if( (pszP = strstr( pszOption, "减%" ) ) != NULL )	sscanf( pszP+3, "%d", &dtemp );
...
#ifdef _PRO3_ADDSKILL
						dec_dex -= dec_dex*(CHAR_getWorkInt( charaindex, CHAR_WORKDEXICEARROW )*0.01);
#endif
   ...
   ★  .  ·● ● ·. .★. 
     ..· /?\_/?\_ ·. 
       ??√√ √√??? 
        中秋节快乐 
*/
void BATTLE_MultiAttMagic_Fire( int battleindex, int attackNo, int defNo,int FieldAttr , int Power)
    ...
    for(i=0;i<listidx;i++){
      ...
      if (charaidx == -1){
      	 print("\n use magic, charaidx = -1");
		  	//int attType2 = CHAR_getInt(BATTLE_No2Index(battleindex,attackNo),CHAR_WHICHTYPE);
		  	//if( attType2 == CHAR_TYPEPLAYER || attType2 == CHAR_TYPEPET)
			  {
