...
// 猎人
	...
	{ "PROFESSION_DARK_SEAL",		PROFESSION_dark_seal,		0 },	// 闇灵封印
...
BOOL PROFESSION_initSkill( char *filename){
	...
	while( fgets( line, sizeof( line ), f ) ){
		...
		if( getStringFromIndexWithDelim( line, ",", PROFESSION_SKILL_DATACHARNUM+PROFESSION_SKILL_ID+1,token, sizeof(token)) == FALSE ) continue;
...
{	
	...
	if(skill_level > 0){
		...
		if( CHAR_getInt( charaindex, PROFESSION_CLASS ) == PROFESSION_CLASS_WIZARD ){
			if( GETHIGHVALUE(CHAR_getWorkInt( charaindex, CHAR_WORK_DARK_SEAL ) ) > 0 )//闇灵封印 回合
				return -1;
		}
#endif
		// 嗜血成性 
...
#endif
#ifdef _PRO_KILLME
int PROFESSION_killme( int charaindex, int toNo, int array, char *data, int skill_level )
{
	profession_common_fun( charaindex, toNo, skill_level, array, BATTLE_COM_S_KILLME );	
	return TRUE;
}
#endif
#ifdef _PRO_ABSOLUTE_DEFENSE
int PROFESSION_absolute_defense( int charaindex, int toNo, int array, char *data, int skill_level )
{
	profession_common_fun( charaindex, toNo, skill_level, array, BATTLE_COM_S_ABSOLUTE_DEFENSE );	
	return TRUE;
}
#endif
#ifdef _PRO3_ADDSKILL
//裂骨断筋
int PROFESSION_crack_bone( int charaindex, int toNo, int array, char *data, int skill_level )
{
	profession_common_fun( charaindex, toNo, skill_level, array, BATTLE_COM_S_CRACK_BONE );	
	return TRUE;
}
//毒素之网
int PROFESSION_poison_dragnet( int charaindex, int toNo, int array, char *data, int skill_level )
{
	profession_common_fun( charaindex, toNo, skill_level, array, BATTLE_COM_S_POISON_DRAGNET );	
	return TRUE;
}
//抚慰心灵
int PROFESSION_pacify( int charaindex, int toNo, int array, char *data, int skill_level )
{
	profession_common_fun( charaindex, toNo, skill_level, array, BATTLE_COM_S_PACIFY );	
	return TRUE;
}
//闇灵封印
int PROFESSION_dark_seal( int charaindex, int toNo, int array, char *data, int skill_level )
{
	profession_common_fun( charaindex, toNo, skill_level, array, BATTLE_COM_S_DARK_SEAL );	
	return TRUE;
}
//斗气导引
int PROFESSION_restate( int charaindex, int toNo, int array, char *data, int skill_level )
{
	profession_common_fun( charaindex, toNo, skill_level, array, BATTLE_COM_S_RESTATE );	
	return TRUE;
}
//恶魔诅咒
int PROFESSION_demon_speech( int charaindex, int toNo, int array, char *data, int skill_level )
{
	profession_common_fun( charaindex, toNo, skill_level, array, BATTLE_COM_S_DEMON_SPEECH );	
	return TRUE;
}
//战场急救
int PROFESSION_first_aid( int charaindex, int toNo, int array, char *data, int skill_level )
{
	profession_common_fun( charaindex, toNo, skill_level, array, BATTLE_COM_S_FIRST_AID );	
	return TRUE;
}
//战狼怒吼
int PROFESSION_roar( int charaindex, int toNo, int array, char *data, int skill_level )
{
	profession_common_fun( charaindex, toNo, skill_level, array, BATTLE_COM_S_PROAR );	
	return TRUE;
}
//制药学
int PROFESSION_medication( int charaindex, int toNo, int array, char *data, int skill_level )
{
	profession_common_fun( charaindex, toNo, skill_level, array, BATTLE_COM_S_MEDICATION );	
	return TRUE;
}
#endif
// 猎人技能-团体火抗性提升
