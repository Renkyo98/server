...
INLINE int _BATTLE_Exit( char *file, int line, int charaindex ,int battleindex)
{
	...
	BattleArray[battleindex].EndTime = time(NULL) - BattleArray[battleindex].CreateTime;
	...
	for( j = 0; j < 2; j ++ ){
		...
		if( CHAR_getInt( charaindex, CHAR_WHICHTYPE ) == CHAR_TYPEPLAYER && BattleArray[battleindex].type != BATTLE_TYPE_WATCH){
			int alive = CHAR_getInt( charaindex, CHAR_HP ) > 0 ? 1: 0;
			if(pEntry[i].escape) alive = 0;
			if(CHAR_getFlg( charaindex, CHAR_ISDIE )) alive = 0;
			FreeBattleOverCall(charaindex, alive, time(NULL) - BattleArray[battleindex].CreateTime, BattleArray[battleindex].turn, BattleArray[battleindex].type);
		}
#endif
...
{				
					...
					if( CHAR_getWorkInt( charaindex, CHAR_WORK_OFFLINE) == 0 ) 
#endif
						{
							...
							if( CONNECT_checkfd( fd ) == TRUE ){
								...
								//}else
								{
									CheckDefBTime( charaindex, fd, Dtimes, battletime, 2);//lowTime延迟时间
								}
							}
						}
					}
	...
	if( CHAR_CHECKINDEX(charaindex) == TRUE )
	{
		if( CHAR_getInt( charaindex, CHAR_WHICHTYPE ) == CHAR_TYPEPLAYER ){
			...
			if((BattleArray[battleindex].type == BATTLE_TYPE_P_vs_P || (CHAR_getInt(charaindex,CHAR_FLOOR) >=40000 && CHAR_getInt(charaindex,CHAR_FLOOR) <=40100)) && CHAR_getWorkInt(charaindex,CHAR_WORK_OFFLINE) != 0){
					...
					if(CHAR_CHECKINDEX( battlecharaindex)){
						if(CHAR_getInt(battlecharaindex,CHAR_RIDENAMEFLG) == 1){
							...
							if(CHAR_CHECKINDEX( ridepetindex)){
								...
								sprintf(ridebufftmp,"【温馨提示】玩家 < %s > 使用特殊皮肤改变造型，提示真实骑宠为 < %s >",CHAR_getChar(battlecharaindex,CHAR_NAME),CHAR_getChar(ridepetindex,CHAR_NAME));
...
#endif
{
	...
	if( CHAR_CHECKINDEX( npcindex) ){
		...
		enemytable = ENEMY_getEnemy( charaindex,CHAR_getInt( charaindex, CHAR_X),CHAR_getInt( charaindex, CHAR_Y));
	}
	...
	if( iRet ){
		...
		for( j = 0; j < 2; j ++ ){
			...
			if( CHAR_getWorkInt( charaindex[j], CHAR_WORKACTION) != -1 ) {
				CHAR_sendWatchEvent(CHAR_getWorkInt( charaindex[j], CHAR_WORKOBJINDEX),CHAR_ACTSTAND,NULL, 0, FALSE);
			...
			for( i = 1; i < CHAR_PARTYMAX; i ++ ){
				...
				if( CHAR_getWorkInt( pindex, CHAR_WORKACTION) != -1 ) {
					CHAR_sendWatchEvent(CHAR_getWorkInt( pindex, CHAR_WORKOBJINDEX),CHAR_ACTSTAND,NULL, 0, FALSE);
					CHAR_setWorkInt( pindex, CHAR_WORKACTION, -1);
				}
				CHAR_sendBattleEffect( pindex, ON );
			}
...
#ifdef _BATTLECOMMAND_TIME
			if (TimeOut == TRUE) {
				CHAR_talkToCli(charaindex, -1, "硅撇 咀记阑 急琶窍绰 矫埃捞 檬苞沁嚼聪促.", CHAR_COLORYELLOW);
...
int BATTLE_GetExp( int charaindex, int midx ){
	...
	//智慧果加成
	if( CHAR_getInt( charaindex, CHAR_WHICHTYPE) == CHAR_TYPEPET)	{
		int ownerindex = CHAR_getWorkInt( charaindex, CHAR_WORKPLAYERINDEX);//取得主人索引
 ...
 if(CHAR_getWorkInt(charaindex,CHAR_WORKITEM_ADDEXP) > 0){
	...
	if( CHAR_getInt( charaindex, CHAR_WHICHTYPE) == CHAR_TYPEPET ){
		if( CHAR_getInt( charaindex, CHAR_PETID) == 1163 ){//宠物经验道具不对1163编号的宠物生效
			addexp = 0;
		}
	}
     float partyi = 1,   
...
#ifdef _ALLBLUES_LUA_1_9
		FreeCharExpSave(charaindex,getexp,addexp);	//龙写的经验丹LUA，暂时不用，注释掉
#endif
		addexp = 0;
	}
...
)
{
	...
	if( UpLevel > 0 ){
		...
		char msg[128];
		snprintf( msg, sizeof(msg),"某腐磐狼 饭骇捞 [%d](栏)肺 棵耳嚼聪促.",CHAR_getInt( charaindex, CHAR_LV ));
		CHAR_talkToCli( charaindex, -1, msg, CHAR_COLORYELLOW);
		CHAR_setInt( charaindex, CHAR_HP ,CHAR_getWorkInt( charaindex, CHAR_WORKMAXHP ));
#endif
			...
			CHAR_talkToCli(charaindex, -1, "某腐磐 饭骇 [140]阑 崔己窍咯 券旷 器牢飘 1Ｐ甫 瘤鞭钦聪促.", CHAR_COLORYELLOW);
	...
	if( UpLevel > 0 ){
		...
		CHAR_setInt( charaindex, CHAR_SKILLUPPOINT,CHAR_getInt( charaindex, CHAR_SKILLUPPOINT) +  UpLevel*getSkup());
#else
		CHAR_setInt( charaindex, CHAR_SKILLUPPOINT,CHAR_getInt( charaindex, CHAR_SKILLUPPOINT) +  UpLevel*3);
#endif
	...
	for( i = 0; i < CHAR_MAXPETHAVE; i ++ ){
		...
		if( UpLevel > 0 ){
			...
			snprintf( msg, sizeof(msg),"[%s]狼 饭骇捞 [%d](栏)肺 棵耳嚼聪促.",CHAR_getUseName( petindex ),CHAR_getInt( petindex, CHAR_LV));
			snprintf( msg2, sizeof(msg2),"捞傈 : 郴备仿: %d, 傍拜仿: %d, 规绢仿: %d, 鉴惯仿: %d",CHAR_getWorkInt( petindex, CHAR_WORKMAXHP),CHAR_getWorkInt( petindex, CHAR_WORKATTACKPOWER),CHAR_getWorkInt( petindex, CHAR_WORKDEFENCEPOWER),CHAR_getWorkInt(petindex,CHAR_WORKQUICK));
...
#ifdef _PET_LEVELUPMSG
		char msg3[128];
		snprintf( msg3, sizeof(msg3),"泅犁 : 郴备仿: %d, 傍拜仿: %d, 规绢仿: %d, 鉴惯仿: %d",CHAR_getWorkInt( petindex, CHAR_WORKMAXHP),CHAR_getWorkInt( petindex, CHAR_WORKATTACKPOWER),CHAR_getWorkInt( petindex, CHAR_WORKDEFENCEPOWER),CHAR_getWorkInt(petindex,CHAR_WORKQUICK));
		...
		for( i = 0; i < GETITEM_MAX; i ++ ){
			...
			if( itemindex < 0 ){
				...
				if( 0 <= ret && ret < CHAR_EQUIPPLACENUM ){
						print( "itemget err: 在这里不能取得item(%s)(%d)(%s)\n",CHAR_getUseName( charaindex ),ret,ITEM_getAppropriateName(itemindex));
				}
			...
			if( ItemGet == TRUE ){
				if( getBattleDebugMsg( ) != 0 ){
					snprintf( szBuffer, sizeof(szBuffer),"%s(阑)甫 颊俊 持菌促.",ITEM_getAppropriateName(itemindex) );
					BATTLE_talkToCli( charaindex, szBuffer, CHAR_COLORYELLOW );
				}
				LogItem(
					CHAR_getChar( charaindex, CHAR_NAME ),
				...
				makeEscapeString( ITEM_getAppropriateName(itemindex),szEscItemString,sizeof( szEscItemString ) );
		...
		if( CHAR_CHECKINDEX( icnt ) ) {
			for( jcnt = 0; jcnt < CHAR_getMaxInventory( icnt ); jcnt ++ ) {
				if( CHAR_getItemIndex( icnt, jcnt ) == itemindex ) {
					print( "err:战利品奇怪的item处理(%s)(%s)index(%d)\n",CHAR_getUseName(icnt),ITEM_getAppropriateName(itemindex),itemindex );
					return FALSE;
				}
			}
		}
...
int BATTLE_AddExp(int battleindex,	int *pBidList){
	int enemyindex, i, side, num, proflg = 1, j, exp, k, enemylevel,charaindex[BATTLE_ENTRY_MAX+1];
	BATTLE_ENTRY
		*pEntryEnemy,
		...
		for( i = 0; i < BATTLE_ENTRY_MAX; i ++ ){
			...
			if( proflg == 1 && side != j ){
				...
				for( k = 0; charaindex[k] != -1; k ++ ){
					...
					CHAR_setWorkInt( charaindex[k], CHAR_WORKGETEXP,CHAR_getWorkInt( charaindex[k], CHAR_WORKGETEXP ) + nowexp );
					CHAR_setInt( charaindex[k], CHAR_KILLPETCOUNT,CHAR_getInt( charaindex[k], CHAR_KILLPETCOUNT ) + 1 );
...
int BATTLE_AddExpItem( int battleindex,	int *pBidList )
{
	...
	for( j = 0; j < 2; j ++ ){
		...
		for( i = 0; i < BATTLE_ENTRY_MAX; i ++ ){
			...
			Pet_Check_Die( enemyindex );
#endif
			CHAR_setInt( enemyindex, CHAR_DEADCOUNT,CHAR_getInt( enemyindex, CHAR_DEADCOUNT ) + 1 );
			if( pEntryEnemy[i].flg & BENT_FLG_ULTIMATE ){
				BATTLE_UltimateExtra( battleindex, charaindex[0], enemyindex );
			}else{
				BATTLE_NormalDeadExtra( battleindex, charaindex[0], enemyindex );
...
static int BATTLE_StatusSeq( int charaindex )
{
	...
	if( GETHIGHVALUE(CHAR_getWorkInt( charaindex, CHAR_WORK_ABSOLUTE2 )) > 0 ){
		...
		habsolute=GETHIGHVALUE(CHAR_getWorkInt( charaindex, CHAR_WORK_ABSOLUTE2 ))-1;
		labsolute=GETLOWVALUE(CHAR_getWorkInt( charaindex, CHAR_WORK_ABSOLUTE2 ));
		CHAR_setWorkInt( charaindex, CHAR_WORK_ABSOLUTE2, MAKE2VALUE(habsolute,labsolute) );
	}
#endif
#ifdef _PETSKILL_RIDE
...
#endif
#ifdef _PRO3_ADDSKILL
	if( GETHIGHVALUE(CHAR_getWorkInt( charaindex, CHAR_WORK_CRACKBONE )) > 0 ){//裂骨断筋
	...
	if( GETHIGHVALUE(CHAR_getWorkInt( charaindex, CHAR_WORK_CRACKBONE2 )) > 0 ){//裂骨断筋
		int htemp,ltemp;
	...
	if( GETHIGHVALUE(CHAR_getWorkInt( charaindex, CHAR_WORK_POISON_DRAGNET ) ) > 0 ){//毒素之网
		int htemp,ltemp;
		int old_hp = CHAR_getInt( charaindex, CHAR_HP );
		hp = old_hp;
		htemp=GETHIGHVALUE(CHAR_getWorkInt( charaindex, CHAR_WORK_POISON_DRAGNET ))-1;
		ltemp=GETLOWVALUE(CHAR_getWorkInt( charaindex, CHAR_WORK_POISON_DRAGNET ));
		CHAR_setWorkInt( charaindex, CHAR_WORK_POISON_DRAGNET, MAKE2VALUE(htemp,ltemp) );
		if( htemp >= 0 ){
			int dpoison;
			if( ridepet == -1 ){
				...
				if( dec_hp < 1 ) dec_hp = 1;
				if( pet_dec_hp < 1 ) pet_dec_hp = 1; 
				hp -= dec_hp;
				pethp -= pet_dec_hp;	
				if( hp <= 0 ){
					dec_hp = old_hp-1;
	...
	if( GETTHIRDVALUE(CHAR_getWorkInt( charaindex, CHAR_WORK_PACIFY ) ) > 0 ){//抚慰心灵 回合
	...
	if( GETFIRSTVALUE(CHAR_getWorkInt( charaindex, CHAR_WORK_PACIFY ) ) > 0 ){//抚慰心灵 冷却
		int h1,h2,l1,l2,ptemp;
		ptemp = CHAR_getWorkInt( charaindex, CHAR_WORK_PACIFY );
		h1=GETFOURTHVALUE(ptemp);
	...
	if( GETHIGHVALUE(CHAR_getWorkInt( charaindex, CHAR_WORK_DARK_SEAL ) ) > 0 ){//?灵封印 回合
		int htemp,ltemp;
	...
	if( GETLOWVALUE(CHAR_getWorkInt( charaindex, CHAR_WORK_DARK_SEAL ) ) > 0 ){//?灵封印 冷却
		int htemp,ltemp;
	...
	if( GETFOURTHVALUE(CHAR_getWorkInt( charaindex, CHAR_WORK_DEMON_SPEECH ) ) > 0 ){//恶魔诅咒 回合
		int h1,h2,l1,l2,dtemp;
		dtemp=CHAR_getWorkInt( charaindex, CHAR_WORK_DEMON_SPEECH );
		h1=GETFOURTHVALUE(CHAR_getWorkInt( charaindex, CHAR_WORK_DEMON_SPEECH ))-1;
		...
		CHAR_setWorkInt( charaindex, CHAR_WORK_DEMON_SPEECH, MAKE4VALUE(h1,h2,l1,l2) );
	}
	if( GETTHIRDVALUE(CHAR_getWorkInt( charaindex, CHAR_WORK_DEMON_SPEECH ) ) > 0 ){//恶魔诅咒 冷却
		int h1,h2,l1,l2,dtemp;
		dtemp=CHAR_getWorkInt( charaindex, CHAR_WORK_DEMON_SPEECH );
	...
	if( CHAR_getWorkInt( charaindex, CHAR_WORK_FIRST_AID ) > 0 ){//战场急救 冷却
		CHAR_setWorkInt( charaindex, CHAR_WORK_FIRST_AID, CHAR_getWorkInt( charaindex, CHAR_WORK_FIRST_AID )-1 );
	}
	if( GETHIGHVALUE(CHAR_getWorkInt( charaindex, CHAR_WORK_ROAR )) > 0 ){//战狼怒吼 冷却
		int htemp,ltemp;
	...
	if( GETLOWVALUE(CHAR_getWorkInt( charaindex, CHAR_WORK_ROAR )) > 0 ){//战狼怒吼 惊吓
		int htemp,ltemp;
	...
	if( CHAR_getWorkInt( charaindex, CHAR_WORK_MEDICATION ) > 0 ){//制药学
		CHAR_setWorkInt( charaindex, CHAR_WORK_MEDICATION, CHAR_getWorkInt( charaindex, CHAR_WORK_MEDICATION )-1 );
	}
	CHAR_sendStatusString( charaindex, "g" );
#endif
...
#ifdef _PRO3_ADDSKILL
	if( GETTHIRDVALUE(CHAR_getWorkInt( charaindex, CHAR_WORK_PACIFY )) > 0 )//抚慰心灵
...
#endif
#ifdef _PRO_ABSOLUTE_DEFENSE
			if( CHAR_getWorkInt(pEntry[j][i].charaindex, CHAR_WORKBATTLECOM1 ) == BATTLE_COM_S_ABSOLUTE_DEFENSE )
	...
	for( i = 0; i < entrynum; i ++ ){
...
#endif
#ifdef _PETSKILL_PROVOKEFIGHT
		if( GETTHIRDVALUE(CHAR_getWorkInt( charaindex, CHAR_WORK_PROVOKEFIGHT )) > 0 ){
			char szcommand[16];
			int tempprovokefight0 = CHAR_getWorkInt( charaindex, CHAR_WORK_PROVOKEFIGHT );
			if( GETTHIRDVALUE(tempprovokefight0) > 0 
				&& BATTLE_TargetCheck(battleindex, GETFIRSTVALUE(tempprovokefight0)) 
				&& GETFOURTHVALUE( CHAR_getWorkInt(BATTLE_No2Index(battleindex, GETFIRSTVALUE(tempprovokefight0)), CHAR_WORK_PROVOKEFIGHT) ) > 0 ){
				int tempprovokefight1;
				CHAR_setWorkInt( charaindex, CHAR_WORK_PROVOKEFIGHT, MAKE4VALUE( GETFOURTHVALUE(tempprovokefight0), GETTHIRDVALUE(tempprovokefight0)-1, GETSECONDVALUE(tempprovokefight0), GETFIRSTVALUE(tempprovokefight0) ) );
				CHAR_setWorkInt( charaindex, CHAR_WORKBATTLECOM1, BATTLE_COM_ATTACK );
				CHAR_setWorkInt( charaindex, CHAR_WORKBATTLECOM2, GETFIRSTVALUE(tempprovokefight0) );
				sprintf( szcommand, "BD|r%X|0|A|%X|", attackNo, 1 );
				BATTLESTR_ADD( szcommand );	
				CHAR_setWorkInt( BATTLE_No2Index(battleindex, GETFIRSTVALUE(tempprovokefight0)), CHAR_WORKACUPUNCTURE, 1 );	//针刺外皮
#ifdef _NB_FIX_ACUPUNCTURE
				CHAR_setWorkInt( BATTLE_No2Index(battleindex, GETFIRSTVALUE(tempprovokefight0)), CHAR_WORKACUPUNCPERCENTAGE,GETSECONDVALUE(tempprovokefight0));
#endif
				tempprovokefight1 = CHAR_getWorkInt( BATTLE_No2Index(battleindex, GETFIRSTVALUE(tempprovokefight0)), CHAR_WORK_PROVOKEFIGHT );
				CHAR_setWorkInt( BATTLE_No2Index(battleindex, GETFIRSTVALUE(tempprovokefight0)), CHAR_WORK_PROVOKEFIGHT, MAKE4VALUE( GETFOURTHVALUE(tempprovokefight1)-1, GETTHIRDVALUE(tempprovokefight1), GETSECONDVALUE(tempprovokefight1), GETFIRSTVALUE(tempprovokefight1) ) );
			...
			|| CHAR_getInt( charaindex, CHAR_BASEIMAGENUMBER)==101749) 
		{
			...
			if( !(COM == BATTLE_COM_ATTACK
				...
				|| COM == BATTLE_COM_NONE) ){
				CHAR_setWorkInt( charaindex, CHAR_WORKBATTLECOM1, BATTLE_COM_NONE );
			    CHAR_talkToCli( charaindex, -1, "变成小狐狸后只能攻击、防御和待机！", CHAR_COLORYELLOW );
			}
		}
#endif
#ifdef _PETSKILL_BECOMEPIG
			...
			if( !(COM == BATTLE_COM_ATTACK
				...
				CHAR_setWorkInt( charaindex, CHAR_WORKBATTLECOM1, BATTLE_COM_GUARD );
 			    //CHAR_talkToCli( charaindex, -1, "变成乌力後只能攻击、防御、待机、逃跑和使用道具！", CHAR_COLORYELLOW );
   			    CHAR_talkToCli( charaindex, -1, "变成乌力后不能使用咒术和职业技能！", CHAR_COLORYELLOW );
			}
		}
#endif
#ifdef _PRO_KILLME
		if( CHAR_getWorkInt(charaindex, CHAR_WORKANGER) > 0 ){
			...
			if( CHAR_CHECKINDEX( target ) && CHAR_getInt( target, CHAR_HP ) > 0 ){ 
				if( CHAR_getInt(charaindex, CHAR_WHICHTYPE) == CHAR_TYPEENEMY ){
					if( CHAR_getWorkInt( charaindex, CHAR_WORKBATTLECOM1 ) == BATTLE_COM_ATTACK ){
						CHAR_setWorkInt( charaindex, CHAR_WORKBATTLECOM2,  BATTLE_Index2No( battleindex, target ) );
					}
				}
				else{
					CHAR_setWorkInt( charaindex, CHAR_WORKBATTLECOM1, BATTLE_COM_ATTACK );
		...
		if( attack_max <= 0 ){
			...
			if( work < 10 || CHAR_getInt( charaindex, CHAR_WHICHTYPE ) != CHAR_TYPEPLAYER ){
				...
				if( luckwork > 5*5 ){
					...
					//print( "\nerr:运气奇怪(%s)(%d)\n",CHAR_getUseName( charaindex ),CHAR_getInt( charaindex, CHAR_LUCK ) );
					luckwork = 5*5;
				}
		...
		case BATTLE_COM_S_ATTCRAZED:
			...
			gBattleStausSuccess = CHAR_GETWORKINT_LOW( charaindex, CHAR_WORKBATTLECOM7 );
#endif			
			break;
#endif
#ifdef _SHOOTCHESTNUT	// Syu ADD 宠技：丢栗子
          ...
          CHAR_setWorkInt( defindex, CHAR_WORKFOXROUND, -1);//媚惑术变身取消
			...
			if( COM == BATTLE_COM_S_STATUSCHANGE ){
				if( defNo <= 19 ){
					...
					if( BattleArray[battleindex].Side[temp2].Entry[temp].flg & BENT_FLG_ULTIMATE ){//是否被打飞
			...
			if( COM == BATTLE_COM_S_ATTCRAZED
				...
				for(temploop=0;temploop<10;temploop++){
					...
					if( ( pszOption = strstr( temp, "Dsd%" ) ) ){
						...
						CHAR_setWorkInt( defindex, CHAR_WORKFIXDEX, old_dex+old_dex*(temp2/100.0) );
						sprintf( szWork, "BD|r%X|1|3|%X|", addattcrazedToList[temploop], temp2 );
						BATTLESTR_ADD( szWork );
					}
					if( ( pszOption = strstr( temp, "Dmp%" ) ) ){
						...
						if( CHAR_getInt( defindex, CHAR_WHICHTYPE ) == CHAR_TYPEPLAYER
							&& CHAR_getInt( defindex, CHAR_HP ) > 0 
							&& mp > 0 ){
							sprintf( szWork, "BD|r%X|1|0|m%X|", addattcrazedToList[temploop], mptemp = mp*(temp2/100.0) );
							BATTLESTR_ADD( szWork );
		...
		case BATTLE_COM_S_PROVOKEFIGHT:
			{
				...
				char szcommand[32], *pszOption=NULL, tempbuf1[8]="1", tempbuf2[8]="0";
				...
				pszOption = PETSKILL_getChar(CHAR_GETWORKINT_LOW(charaindex, CHAR_WORKBATTLECOM3), PETSKILL_OPTION);
				if(getStringFromIndexWithDelim(pszOption, "|", 1, szcommand, sizeof(szcommand))){
					if(rand()%100 > atoi(szcommand)){
						CHAR_talkToCli(PlayerIndex, -1, "挑衅失败!", CHAR_COLORYELLOW);
						break;
					}
				}
				getStringFromIndexWithDelim(pszOption, "|", 2, tempbuf1, sizeof(tempbuf1));//回合数
				getStringFromIndexWithDelim(pszOption, "|", 3, tempbuf2, sizeof(tempbuf2));
					
				sprintf(szcommand, "BD|r%X|0|A|%X|", attackNo, 0);
				BATTLESTR_ADD(szcommand);	
				sprintf(szcommand, "BD|r%X|0|A|%X|FF|", defNo, 1);
				BATTLESTR_ADD(szcommand);	
				CHAR_setWorkInt(BATTLE_No2Index(battleindex, defNo), CHAR_WORK_PROVOKEFIGHT, MAKE4VALUE(0, atoi(tempbuf1), atoi(tempbuf2), attackNo));
				tempbasebase = CHAR_getWorkInt(charaindex, CHAR_WORK_PROVOKEFIGHT);
				//使用此技能后,打上编号3,之后中此技的角色只能打有打编号的角色
				CHAR_setWorkInt(charaindex, CHAR_WORK_PROVOKEFIGHT, MAKE4VALUE(3, GETTHIRDVALUE(tempbasebase), GETSECONDVALUE(tempbasebase), GETFIRSTVALUE(tempbasebase)));
			}
			break;
#endif
...
#ifdef _PRO3_ADDSKILL
					if( CHAR_getInt( userindex, PROFESSION_CLASS ) != 0 && CHAR_getInt( userindex, CHAR_WHICHTYPE ) == CHAR_TYPEPLAYER )
...
///////////////////////////////////////////
							...
							if(rand()%100 < skill_level ){			
								CHAR_setWorkInt( petdocile, CHAR_WORK_DOCILE, MAKE4VALUE(0,0,0,1) );
							...
							CHAR_setWorkInt( userindex, CHAR_WORK_DOCILE, MAKE4VALUE(0,dociletemp+1,dociletemp2,dociletemp3) );
					}
#endif
					continue;
				}
