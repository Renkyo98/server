...
void NPC_ItemStrStr(int itemID,double rate,char *name,char *token2,int iCostFame,int iChangeItemCost, int iEventPoint);
...
BOOL NPC_AddItemBuy(int meindex, int talker,int itemID,int kosuu,double rate,int iCostFame,int iChangeItemCost, int EventPoint);
...
{      
  ...
  char buff[256];
	...
	int point;
	char buff3[256];
	if(NPC_Util_GetStrFromStrWithDelim(argstr, "EventPoint", buff3, sizeof(buff3)) != NULL){
...
void NPC_GetItemList(char *argstr,char *argtoken)
{
	...
	if (NPC_Util_GetStrFromStrWithDelim(argstr, "EventPoint", buff7, sizeof(buff7)) == NULL) iEventPoint = -1;
	if( NPC_Util_GetStrFromStrWithDelim( argstr, "ItemList", buff, sizeof( buff)) != NULL ){
	    while( getStringFromIndexWithDelim(buff,",",i,buff2,sizeof(buff2)) !=FALSE ) {
				...
				if (iEventPoint > -1) {
				if (getStringFromIndexWithDelim(buff7, ",", i, buff8, sizeof(buff8)) != FALSE) iEventPoint = atoi(buff8);
				}
			i++;
			if(strstr( buff2, "-") == NULL) {
				...
				NPC_ItemStrStr( atoi( buff2), rate, name, token2,iCostFame,iChangeItemCost, iEventPoint);
	    	strncat( argtoken, token2, sizeof(token2));
				...
				for(; start < end ; start++ ) {
					...
					loopcnt++;
					if(loopcnt == MAXSHOPITEM) break;
					NPC_ItemStrStr( start, rate, name, token2,iCostFame,iChangeItemCost, iEventPoint);
		    	strncat( argtoken, token2, sizeof(token2));
				}
			}
		}
...
void NPC_ItemStrStr(int itemID,double rate,char *name,char *token2,int iCostFame,int iChangeItemCost,int iEventPoint)
{
	...
	if(iChangeItemCost > -1) iCostFame = iChangeItemCost;
	else if (iEventPoint > -1) iCostFame = iEventPoint;
...
BOOL NPC_SetNewItem(int meindex,int talker,char *data)
{
	...
	if( NPC_Util_GetStrFromStrWithDelim( argstr, "ItemList", buf, sizeof( buf)) != NULL ){
		while(getStringFromIndexWithDelim(buf , "," , j, buff2, sizeof(buff2)) != FALSE ){
			...
			if(iChangeItemCost > -1){
				if(getStringFromIndexWithDelim(buff7,",",j,buff8,sizeof(buff8)) != FALSE) iEventPoint = atoi(buff8);
			}
			j++;
			/*--  "-"互殖引木化中月井升丹井--*/
			if(strstr( buff2, "-") == NULL) {
				if( ITEM_getcostFromITEMtabl(atoi(buff2)) !=-1) {
					if ( i == select) {
						/*---失奶  丞及综岳---*/
						/*--蜊醒坌综岳--*/
						if(NPC_AddItemBuy(meindex, talker,atoi(buff2),kosuu,rate,iCostFame,iChangeItemCost, iEventPoint) != TRUE)
						{
							return FALSE;
						}
						return TRUE;
				...
				for(; start < end ; start++ ) {
					if( ITEM_getcostFromITEMtabl( start) != -1) {
						if ( i == select) {
							/*---失奶  丞及综岳---*/
							/*--蜊醒坌综岳--*/
							if(NPC_AddItemBuy(meindex, talker, start, kosuu, rate,iCostFame,iChangeItemCost, iEventPoint) != TRUE)
							{
								return FALSE;
							}
							return TRUE;
 ...
 *失奶  丞及馨笛毛垫丹
 *--------------------------------------------*/
BOOL NPC_AddItemBuy(int meindex, int talker,int itemID,int kosuu,double rate,int iCostFame,int iChangeItemCost, int iEventPoint)
{
	...
	if (iTotalEventPoint > 0) {
		...
		if (CHAR_getInt(talker, CHAR_EVENTPOINT) < iEventPoint) return FALSE;
	}
...
*/
	...
	if (iTotalEventPoint > 0) {
		CHAR_setInt(talker, CHAR_EVENTPOINT, CHAR_getInt(talker, CHAR_EVENTPOINT) - iTotalEventPoint);
