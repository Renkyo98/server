...
*/
	...
	if( strcmp( funcname, "ACShowMemberList") == 0){
		...
		saacproto_ACShowMemberList_recv(fd, result, index, fmmemnum, fmacceptflag, fmjoinnum, data,badge);
#else
  ...
  if( strcmp(funcname, "EFFECT") == 0){
	...
	if( strcmp( funcname , "GMSERVERLIST" ) == 0 ){
		...
		msg = saacproto_wrapStringAddr(saacproto_stringwrapper[3], saacproto.workbufsize, saacproto_demkstr_string(saacproto.token_list[3]));
		printf("\n接收到跨服指令，编号:%d 参数:%s\n",itype,msg);
...
void saacproto_gm_shutdown_send(int itype,char* msg){
	...
	saacproto_strcatsafe( saacproto.work , saacproto_mkstr_int( itype ) ,saacproto.workbufsize );//传递类型编号
	saacproto_strcatsafe( saacproto.work , saacproto_mkstr_string( msg ) ,saacproto.workbufsize );//传递参数
...
void saacproto_playmax_send(char* ip){
	...
	saacproto_strcatsafe( saacproto.work , saacproto_mkstr_string( ip ) ,saacproto.workbufsize );//传递参数
	saacproto_Send( acfd , saacproto.work );//发送至SAAC
}

void saacproto_getplaymax_send(int maxnum){
	saacproto_CreateHeader( saacproto.work , "GETPLAYERMAX" );
	saacproto_strcatsafe( saacproto.work , saacproto_mkstr_int( maxnum ) ,saacproto.workbufsize );//传递类型编号
	saacproto_Send( acfd , saacproto.work );//发送至SAAC
}
#endif
