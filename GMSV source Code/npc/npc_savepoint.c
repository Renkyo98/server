	...
	if(NPC_CheckFlg(CHAR_getInt(talkerindex,CHAR_SAVEPOINT),oldmanid)==FALSE){
...
#ifdef _SAVE_GAME_ID
				if (dd>0) {
					snprintf( timeMsg, sizeof( timeMsg ) ,"[%s]辑滚备悼矫埃 [%d]老 [%d]矫埃 [%d]盒 [%d]檬  ",getGameservername(),dd,hh,mm,ss);
				} else if (hh>0) {
					snprintf( timeMsg, sizeof( timeMsg ) ,"[%s]辑滚备悼矫埃 [%d]矫埃 [%d]盒 [%d]檬  ",getGameservername(),hh,mm,ss);
				} else {
					snprintf( timeMsg, sizeof( timeMsg ) ,"[%s]辑滚备悼矫埃 [%d]盒 [%d]檬  ",getGameservername(),mm,ss);
				}
				strcpy(buf3,timeMsg);
#else
				if (dd>0) {
					snprintf( timeMsg, sizeof( timeMsg ) ,"辑滚备悼矫埃 [%d]老 [%d]矫埃 [%d]盒 [%d]檬  ",dd,hh,mm,ss);
				} else if (hh>0) {
					snprintf( timeMsg, sizeof( timeMsg ) ,"辑滚备悼矫埃 [%d]矫埃 [%d]盒 [%d]檬  ",hh,mm,ss);
				} else {
					snprintf( timeMsg, sizeof( timeMsg ) ,"辑滚备悼矫埃 [%d]盒 [%d]檬  ",mm,ss);
				}
				strcpy(buf3,timeMsg);
#endif
			}
	    {
	    	char temp[1024];
				...
				if(getPlayerNum()>0){
					sprintf(temp,"立加牢盔 [%d]疙", abs(StateTable[LOGIN]) + luaplayernum + getPlayerNum());
				}else{
					sprintf(temp,"立加牢盔 [%d]疙", abs(StateTable[LOGIN]) + luaplayernum);
				}
#else
				extern unsigned int StateTable[];
				if(getPlayerNum()>0){
					sprintf(temp,"立加牢盔 [%d]疙",abs(StateTable[LOGIN]) + getPlayerNum() + rand() % 10);
				}else{
					sprintf(temp,"立加牢盔 [%d]疙",abs(StateTable[LOGIN]));
				}
#endif	
#else
				extern unsigned int StateTable[];
