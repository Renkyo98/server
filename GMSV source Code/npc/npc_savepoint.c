	...
	if(NPC_CheckFlg(CHAR_getInt(talkerindex,CHAR_SAVEPOINT),oldmanid)==FALSE){
...
#ifdef _SAVE_GAME_ID
				if (dd>0) {
					snprintf( timeMsg, sizeof( timeMsg ) ,"[%s]º≠πˆ±∏µøΩ√∞£ [%d]¿œ [%d]Ω√∞£ [%d]∫– [%d]√   ",getGameservername(),dd,hh,mm,ss);
				} else if (hh>0) {
					snprintf( timeMsg, sizeof( timeMsg ) ,"[%s]º≠πˆ±∏µøΩ√∞£ [%d]Ω√∞£ [%d]∫– [%d]√   ",getGameservername(),hh,mm,ss);
				} else {
					snprintf( timeMsg, sizeof( timeMsg ) ,"[%s]º≠πˆ±∏µøΩ√∞£ [%d]∫– [%d]√   ",getGameservername(),mm,ss);
				}
				strcpy(buf3,timeMsg);
#else
				if (dd>0) {
					snprintf( timeMsg, sizeof( timeMsg ) ,"º≠πˆ±∏µøΩ√∞£ [%d]¿œ [%d]Ω√∞£ [%d]∫– [%d]√   ",dd,hh,mm,ss);
				} else if (hh>0) {
					snprintf( timeMsg, sizeof( timeMsg ) ,"º≠πˆ±∏µøΩ√∞£ [%d]Ω√∞£ [%d]∫– [%d]√   ",hh,mm,ss);
				} else {
					snprintf( timeMsg, sizeof( timeMsg ) ,"º≠πˆ±∏µøΩ√∞£ [%d]∫– [%d]√   ",mm,ss);
				}
				strcpy(buf3,timeMsg);
#endif
			}
	    {
	    	char temp[1024];
				...
				if(getPlayerNum()>0){
					sprintf(temp,"¡¢º”¿Œø¯ [%d]∏Ì", abs(StateTable[LOGIN]) + luaplayernum + getPlayerNum());
				}else{
					sprintf(temp,"¡¢º”¿Œø¯ [%d]∏Ì", abs(StateTable[LOGIN]) + luaplayernum);
				}
#else
				extern unsigned int StateTable[];
				if(getPlayerNum()>0){
					sprintf(temp,"¡¢º”¿Œø¯ [%d]∏Ì",abs(StateTable[LOGIN]) + getPlayerNum() + rand() % 10);
				}else{
					sprintf(temp,"¡¢º”¿Œø¯ [%d]∏Ì",abs(StateTable[LOGIN]));
				}
#endif	
#else
				extern unsigned int StateTable[];
...
BOOL NPC_UsedCheck(int meindex,int talker,int flg)
{
  ...
  } else return TRUE;   // Arminius 2.5 »Áπ˚√ª”–…Ë GetItem ‘Ú“ª¬…¥¢¥Ê
