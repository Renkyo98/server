			...
			if (CHAR_CHECKINDEX(toindex)) {
				...
				if (emptyitemindexinchara >= 0) {
					...
					if (itemindex > -1) {
						...
						char token[256];
						sprintf(token, "掘菌嚼聪促 : %s", ITEM_getChar(itemindex, ITEM_NAME));
		...
		for (i = 0; i < 5; i++) {
			...
			if (CHAR_CHECKINDEX(toindex)) {
				...
				if (strstr(argstr, "NEWNPCENEMY") ) {
					...
					if (NPC_Util_GetStrFromStrWithDelim(argstr, "PROBCHANCE", buffer, sizeof(buffer)) != FALSE) {
						prob = atoi(buffer);
					}

					if (NPC_Util_GetStrFromStrWithDelim(argstr, "TIMECHANCE", buffer, sizeof(buffer)) != FALSE) {
						timesec = atoi(buffer);
					}

					if (prob >= chance)
					{
						if (NPC_Util_GetStrFromStrWithDelim(argstr, "WARPCHANCE", buffer, sizeof(buffer)) != FALSE) {

							getStringFromIndexWithDelim(buffer, ",", 1, buffer2, sizeof(buffer2));
							fl = atoi(buffer2);
							getStringFromIndexWithDelim(buffer, ",", 2, buffer2, sizeof(buffer2));
							x = atoi(buffer2);
							getStringFromIndexWithDelim(buffer, ",", 3, buffer2, sizeof(buffer2));
							y = atoi(buffer2);
						}
						...
						if (CHAR_getWorkInt(toindex, CHAR_WORKPARTYMODE) == CHAR_PARTY_LEADER) {
							...
							for (i = 0; i < CHAR_PARTYMAX; i++) {
								int subindex = CHAR_getWorkInt(toindex, CHAR_WORKPARTYINDEX1 + i);
					...
					{
						if (NPC_Util_GetStrFromStrWithDelim(argstr, "WARP", buffer, sizeof(buffer)) != FALSE) {
...
BOOL NPC_NPCEnemy_BattleIn(  int meindex, int charaindex)
{
	...
	if( ret == 0 ) {
		...
		lssproto_BattleButton_send(getfdFromCharaIndex( charaindex ), CHAR_getWorkInt(charaindex, CHAR_WORK_NOHELP) ,CHAR_getWorkInt(charaindex, CHAR_WORK_NOESCAPE));
		//组队
		if( CHAR_getWorkInt( charaindex, CHAR_WORKPARTYMODE) == CHAR_PARTY_LEADER ) {
			...
			for( i = 1; i < CHAR_PARTYMAX; i ++ ) {
				...
				if( CHAR_CHECKINDEX(toindex) ) {
					...
					CHAR_setWorkInt( toindex, CHAR_WORK_NOHELP, 1);
					lssproto_BattleButton_send(getfdFromCharaIndex( toindex ), CHAR_getWorkInt(toindex, CHAR_WORK_NOHELP) ,CHAR_getWorkInt(toindex, CHAR_WORK_NOESCAPE));
				}
			}
		}
#endif
