...
#endif
				       getStringFromIndexWithDelim(fmpointlist.pointlistarray[fmidtmp], "|", 5, fmindextmp, sizeof(fmindextmp));
	...
	case CHAR_WINDOWTYPE_WINDOWWARPMAN_MAIN:
		if(select==WINDOW_BUTTONTYPE_YES){
			...
			if ((fmpks[fmpks_pos].host_index != -1)
				&& (CHAR_getWorkInt(talkerindex, CHAR_WORKFMINDEXI) == fmpks[fmpks_pos].host_index))
			{
				...
				&& (CHAR_getWorkInt(talkerindex, CHAR_WORKFMINDEXI) == fmpks[fmpks_pos].guest_index))
			{
				CHECK_FMPknumInFloor( meindex);
				if( CHAR_getWorkInt( meindex, NPC_WORK_FMNUMII) > (fmpks[fmpks_pos].max_player - 1)){
					lssproto_WN_send( fd, WINDOW_MESSAGETYPE_MESSAGE,
						WINDOW_BUTTONTYPE_OK, -1, -1,
						makeEscapeString("已经不能再进去啰～！\n家族进场人数已经到达设定上限了！", token, sizeof(token)));
					return;
				}
			}
			//修复族战人数超量结束
