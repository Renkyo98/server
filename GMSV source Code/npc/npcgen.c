...
static BOOL NPC_generateNPC( int createindex, int createtemplateindex )
{
    ...
    if( cr->chardata[NPC_CREATENEWNAME].string[0] != '\0' ){
        strcpysafe(one.string[CHAR_NB_NAME].string,sizeof(one.string[CHAR_NB_NAME].string),cr->chardata[NPC_CREATENEWNAME].string);
    }
    ...
    if( cr->arg[createtemplateindex].string[0] != '\0'  ){
        strcpysafe( one.string[CHAR_NPCARGUMENT].string,sizeof(one.string[CHAR_NPCARGUMENT].string),cr->arg[createtemplateindex].string );
    }else{
        ...
        for( i = 0 ; i < template->intdata[NPC_TEMPLATEITEMNUM] ; i ++ ){
            if( RAND(0,32767) <= template->haveitem[i].haverate ){
                if( template->haveitem[i].itemnumber == -1 && template->haveitem[i].havenum > 0 ){
                    ...
                    one.indexOfExistItems[itemindex+i] =ITEM_makeItemAndRegist(template->haveitem[i].itemnumber);
                }
            }
        }
    }
