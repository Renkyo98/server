    ...
    if( CHAR_getInt( petindex, CHAR_PETID) == 718
		...
		|| CHAR_getInt( petindex, CHAR_PETID) == 401
		  ...
		  if( CHAR_getInt( talkerindex, CHAR_LV) < 80 )	{
				CHAR_talkToCli( talkerindex, -1, "??? 80 ????? ??? ??? ? ????.！",  CHAR_COLORWHITE);
				return;
...
BOOL NPC_TransmigrationMain(int meindex, int toindex, char *buf)
{
	...
	//if(CHAR_getInt(toindex, CHAR_SKILLUPPOINT) > 0) { //修复转生异点
		//CHAR_talkToCli( toindex,-1,"你身上的点数没加完，所以我不能让你转生！",CHAR_COLORWHITE); 
		//return FALSE;
	//}
...
//取array=========================
	...
	for(count=0;count<CHAR_MAXPETHAVE;count++)	{
		if(CHAR_getCharPet(meindex,count) == ret)	{	
			CHAR_send_K_StatusString(meindex,count,CHAR_K_STRING_HP|CHAR_K_STRING_AI);
		}
	}
  sprintf(token,"%s 接受了精灵王的祝福！", CHAR_getUseName( ret));
