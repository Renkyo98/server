...
void NPC_PetSkillShopWindowTalked( int meindex, int talkerindex,int seqno, int select, char *data)
{
...
#ifdef _NB_PETKILL_COST
	iCostFame = 0;
	if (NPC_Util_GetStrFromStrWithDelim(argstr, "cost_fame", msg, sizeof(msg)) != NULL){
	...
	if (CHAR_getInt(talkerindex, CHAR_FAME) < iCostFame)
		return;
	CHAR_setInt(talkerindex, CHAR_FAME, CHAR_getInt(talkerindex, CHAR_FAME) - iCostFame);
	if (CHAR_getInt(talkerindex, CHAR_FMINDEX) != -1 && (strcmp(CHAR_getChar(talkerindex, CHAR_FMNAME),""))){
			int		fd = getfdFromCharaIndex(talkerindex);
			char	buf[256];
			sprintf(buf, "%d", CHAR_getInt(talkerindex, CHAR_FAME));
			saacproto_ACFixFMData_send(acfd, CHAR_getChar(talkerindex, CHAR_FMNAME), CHAR_getInt(talkerindex, CHAR_FMINDEX),
									   CHAR_getWorkInt(talkerindex, CHAR_WORKFMINDEXI), FM_FIX_FAME, buf, "",
									   CHAR_getWorkInt(talkerindex, CHAR_WORKFMCHARINDEX), CONNECT_getFdid(fd));
	}
#endif
...
#ifdef _NB_PETKILL_COST
			iCostFame = 0;
			if (NPC_Util_GetStrFromStrWithDelim(argstr, "cost_fame", msg2, sizeof(msg2)) != NULL){
				if (getStringFromIndexWithDelim(msg2, ",", i, buf, sizeof(buf)) != FALSE)
					iCostFame = atoi(buf);
			}
			sprintf(token2,"|%s|%d|%s|%d", PETSKILL_getChar(skillarray, PETSKILL_NAME), cost,
				PETSKILL_getChar(skillarray, PETSKILL_COMMENT), iCostFame);
#else
			sprintf(token2,"|%s|%d|%s", PETSKILL_getChar(skillarray, PETSKILL_NAME), cost,
