...
void NPC_PoolItemShopWindowTalked( int meindex, int talkerindex, int seqno, int select, char *data)
{
...
//	print("\n NPC_PoolItemShopWindowTalked: seq:%d sel:%d data:%s", seqno, select, data);
	...
	if( ITEM_getITEM_UseItemnum() >= getItemnum()-100 ){//Change fix 防止记忆体满了后,仓库道具会消失
		CHAR_talkToCli( talkerindex, -1, "目前忙碌中，无法取出道具，建议至其它伺服器再试！", CHAR_COLORYELLOW);
		return;
	}
#endif
		...
		switch( atoi( data)) {
		  ...
		  case 3:
			if( !CHAR_CheckDepotItem( talkerindex) ){
				...
				CHAR_talkToCli( talkerindex, -1, "拌沥芒绊，立加己傍！", CHAR_COLORYELLOW);
			}else{
	  ...
	  case CHAR_WINDOWTYPE_DEPOTITEMSHOP_ADD://放入
			...
			if( atoi( data) == 0){
				...
				if( NPC_DepotItem_InsertItem( meindex, talkerindex, atoi( data)-1) == FALSE ){
					NPC_PoolItemShop_DepotItem_Menu( meindex, talkerindex);
					CHAR_talkToCli( talkerindex, -1, "存放道具失败，请稍后再试！", CHAR_COLORYELLOW);
				}
/*
#ifdef _NB_FIX_POOLITEM
				if( !CHAR_CheckDepotItem( talkerindex) ){
					CHAR_GetDepotItem( meindex, talkerindex);
					//CHAR_talkToCli( talkerindex, -1, "取得道具，请稍后！", CHAR_COLORYELLOW);
				}else{
					NPC_PoolItemShop_DepotItem_Menu( meindex, talkerindex);
				}
#endif*/
	  ...
	  case CHAR_WINDOWTYPE_DEPOTITEMSHOP_GET:
			...
			if( atoi( data) == 0){
				...
				if( NPC_DepotItem_gettItem( meindex, talkerindex, atoi( data)-1) == FALSE ){
					NPC_PoolItemShop_DepotItem_Menu( meindex, talkerindex);
					CHAR_talkToCli( talkerindex, -1, "取出道具失败，请稍后再试！", CHAR_COLORYELLOW);
				}
/*
#ifdef _NB_FIX_POOLITEM
				if( !CHAR_CheckDepotItem( talkerindex) ){
...
{
	char token[256];
	sprintf( token, "仓库道具%s，编码重复！(系统清除)",ITEM_getChar( itemindex, ITEM_NAME));
