...
void NPC_ItemStrStr(int itemID,double rate,char *name,char *token2,int iCostFame,int iChangeItemCost, int eventcost);
...
BOOL NPC_AddItemBuy(int meindex, int talker,int itemID,int kosuu,double rate,int iCostFame,int iChangeItemCost, int eventcost);
...
{      
  ...
  char buff[256];
	...
	if( NPC_Util_GetStrFromStrWithDelim( argstr, "CostEvent", buff, sizeof( buff)) != NULL )
	{
...
void NPC_GetItemList(char *argstr,char *argtoken)
{
	...
	if(NPC_Util_GetStrFromStrWithDelim(argstr,"CostEvent",buff7,sizeof(buff7)) == NULL) eventcost = -1;
	if( NPC_Util_GetStrFromStrWithDelim( argstr, "ItemList", buff, sizeof( buff)) != NULL ){
	    while( getStringFromIndexWithDelim(buff,",",i,buff2,sizeof(buff2)) !=FALSE ) {
				...
				if(eventcost > -1){
					if(getStringFromIndexWithDelim(buff7,",",i,buff8,sizeof(buff8)) != FALSE) eventcost = atoi(buff8);
				}
			i++;
			if(strstr( buff2, "-") == NULL) {
				...
				NPC_ItemStrStr( atoi( buff2), rate, name, token2,iCostFame,iChangeItemCost, eventcost);
	    	strncat( argtoken, token2, sizeof(token2));
				...
				for(; start < end ; start++ ) {
					...
					loopcnt++;
					if(loopcnt == MAXSHOPITEM) break;
					NPC_ItemStrStr( start, rate, name, token2,iCostFame,iChangeItemCost, eventcost);
		    	strncat( argtoken, token2, sizeof(token2));
				}
			}
		}
...
void NPC_ItemStrStr(int itemID,double rate,char *name,char *token2,int iCostFame,int iChangeItemCost, int eventcost)
{
	...
	else if( eventcost > -1 ) gold = eventcost;
	else gold  = ITEM_getcostFromITEMtabl( itemID);
	...
	if( eventcost > -1 )
	{
		sprintf(token2,"%s|0|%d|%d|%d|%s|%d|",escape,level,gold,graNo,info,eventcost < 0 ? -1:eventcost);
	}
	else
	{
...
BOOL NPC_SetNewItem(int meindex,int talker,char *data)
{
	...
	if( NPC_Util_GetStrFromStrWithDelim( argstr, "ItemList", buf, sizeof( buf)) != NULL ){
		while(getStringFromIndexWithDelim(buf , "," , j, buff2, sizeof(buff2)) != FALSE ){
			...
			if( eventcost > -1 ){
				if(getStringFromIndexWithDelim(buff7,",",j,buff8,sizeof(buff8)) != FALSE) eventcost = atoi(buff8);
			}
			j++;
			/*--  "-"互殖引木化中月井升丹井--*/
			if(strstr( buff2, "-") == NULL) {
				if( ITEM_getcostFromITEMtabl(atoi(buff2)) !=-1) {
					if ( i == select) {
						/*---失奶  丞及综岳---*/
						/*--蜊醒坌综岳--*/
						if(NPC_AddItemBuy(meindex, talker,atoi(buff2),kosuu,rate,iCostFame,iChangeItemCost,eventcost) != TRUE)
						{
							return FALSE;
						}
						return TRUE;
				...
				for(; start < end ; start++ ) {
					if( ITEM_getcostFromITEMtabl( start) != -1) {
						if ( i == select) {
							/*---失奶  丞及综岳---*/
							/*--蜊醒坌综岳--*/
							if(NPC_AddItemBuy(meindex, talker, start, kosuu, rate,iCostFame,iChangeItemCost,eventcost) != TRUE)
							{
								return FALSE;
							}
							return TRUE;
...
BOOL NPC_AddItemBuy(int meindex, int talker,int itemID,int kosuu,double rate,int iCostFame,int iChangeItemCost, int eventcost)
{
	...
	else
	{
		if(CHAR_getInt( talker, CHAR_GOLD) < maxgold ){
			CHAR_talkToCli( talker, -1, "您的石币不足，无法购买此物品。", CHAR_COLORYELLOW);
			return FALSE;
		} 
		...
		if(iCostFame > 0){
			...
			if(CHAR_getInt(talker,CHAR_FAME) < iTotalCostFame){
				CHAR_talkToCli( talker, -1, "您的声望不足，无法购买此物品。", CHAR_COLORYELLOW);
				return FALSE;
			} 
		}
	}
...
*/
	...
	if( eventcost > 0 ) 
	{
		CHAR_setInt( talker, CHAR_EVENTPOINT, CHAR_getInt(talker, CHAR_EVENTPOINT) - maxgold );
	...
	else
	{
		...
		char token[256];
		sprintf(token, "酒捞袍 [%s](阑)甫 %d俺 备概窍咯 %d胶沛阑 荤侩沁嚼聪促。 唱狼 胶沛: %d胶沛", ITEM_getChar(itemindex, ITEM_NAME), kosuu, maxgold, CHAR_getInt(talker, CHAR_GOLD));
		CHAR_talkToCli(talker, -1, token, CHAR_COLORYELLOW);
	}
...
{	
	...
	NPC_Util_GetStrFromStrWithDelim( argstr, "main_msg", buff, sizeof( buff));
#ifdef USE_EVENTPOINT
	if( NPC_Util_GetStrFromStrWithDelim( argstr, "CostEvent", buff2, sizeof( buff2)) != NULL )
	{
		snprintf(token, sizeof(token),"%s|%s|%d",CHAR_getChar(meindex,CHAR_NAME),buff,CHAR_getInt(talker,CHAR_EVENTPOINT));
	}
	else
	{
...
#ifdef USE_EVENTPOINT
	char _buffer[1024];
	if( NPC_Util_GetStrFromStrWithDelim( argstr, "CostEvent", _buffer, sizeof( _buffer)) != NULL )
	{
		sprintf(token,"FAME|%d",CHAR_getInt(talker,CHAR_EVENTPOINT));
		lssproto_S2_send(fd,token);
	}
...
BOOL NPC_SellNewItem(int meindex,int talker,char *data)
{
	...
	if (select < CHAR_STARTITEMARRAY || select >= CHAR_getMaxInventory(talker)) return FALSE;
	k = select;
	...
	int maxpile = CHAR_getInt(talker, CHAR_TRANSMIGRATION) + (CHAR_getInt(talker, CHAR_TRANSMIGRATION) / 5) * 2 + 3;
