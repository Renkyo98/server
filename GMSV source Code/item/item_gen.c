...
int ITEM_mergeItem( int charaindex, ITEM_Item *items, int num , int money, int petid, int searchtable, int petindex, int alchemist)
{
	...
	if( nowtime - CHAR_getWorkInt( charaindex, CHAR_WORKLASTMERGETIME) < 1 ) {
		...
		CHAR_talkToCli( charaindex, -1 ,"合成料理过于频繁，休息一下比较好哟。", CHAR_COLORRED);
		//print(" 合成频繁 ");
		...
		if( CHAR_CHECKITEMINDEX( charaindex, haveitemindex) ) {
			...
			if( ITEM_CHECKINDEX( itemindex)) {
				...
				if (ITEM_getInt(itemindex,ITEM_ItemLock) == 1){
					CHAR_talkToCli( charaindex, -1, "【温馨提示】绑定的物品无法合成。", CHAR_COLORYELLOW );
					return FALSE;
				}
#endif
#ifdef _ITEM_INSLAY
						...
						if( CHAR_getInt( ownerindex, CHAR_FMLEADERFLAG) != FMMEMBER_LEADER)
							...
							if( randtable == 1){
								...
								if( ITEM_getInt( createitemindex, ITEM_TYPE) == 20){								
									...
									//Change fix 原本少加了忠诚的功能,结果要登出入后,道具才有加忠诚效果,这里补上 2005/01/04
									if( (p = strstr( ITEM_getChar(createitemindex, ITEM_ARGUMENT), "忠")) ){
										strcpy( argbuf, (p+2));
										arg3 = atoi(argbuf);
...
char *magicname;
				char nowstr[64], nowtgh[64], nowdex[64], nowvit[64], nowcharm[64], nowcriti[64], nowyoid[64], Mname[64], set[256], data[64], nowpenetration[64], nowshield[64];
				...
				sprintf(nowpenetration, "%d", ITEM_getInt(createitemindex, ITEM_OTHERDAMAGE)); 
				sprintf(nowshield, "%d", ITEM_getInt(createitemindex, ITEM_OTHERDEFC));        

				memset(Mname, 0, sizeof(Mname));
				if (ITEM_getInt(createitemindex, ITEM_MAGICID) > 0) { 
					int marray = MAGIC_getMagicArray(ITEM_getInt(createitemindex, ITEM_MAGICID)); 
					if (marray != -1) { 
				...
				sprintf(set, "%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s %s",
					(ITEM_getInt(createitemindex, ITEM_MODIFYATTACK) != 0) ? "傍" : "", (ITEM_getInt(createitemindex, ITEM_MODIFYATTACK) > 0) ? "+" : "", (ITEM_getInt(createitemindex, ITEM_MODIFYATTACK) != 0) ? nowstr : "", (ITEM_getInt(createitemindex, ITEM_MODIFYATTACK) != 0) ? " " : "",
					(ITEM_getInt(createitemindex, ITEM_MODIFYDEFENCE) != 0) ? "规" : "", (ITEM_getInt(createitemindex, ITEM_MODIFYDEFENCE) > 0) ? "+" : "", (ITEM_getInt(createitemindex, ITEM_MODIFYDEFENCE) != 0) ? nowtgh : "",  (ITEM_getInt(createitemindex, ITEM_MODIFYDEFENCE) != 0) ? " " : "",
					(ITEM_getInt(createitemindex, ITEM_MODIFYQUICK) != 0) ? "刮" : "", (ITEM_getInt(createitemindex, ITEM_MODIFYQUICK) > 0) ? "+" : "", (ITEM_getInt(createitemindex, ITEM_MODIFYQUICK) != 0) ? nowdex : "", (ITEM_getInt(createitemindex, ITEM_MODIFYQUICK) != 0) ? " " : "",
					(ITEM_getInt(createitemindex, ITEM_MODIFYHP) != 0) ? "郴" : "", (ITEM_getInt(createitemindex, ITEM_MODIFYHP) > 0) ? "+" : "", (ITEM_getInt(createitemindex, ITEM_MODIFYHP) != 0) ? nowvit : "",  (ITEM_getInt(createitemindex, ITEM_MODIFYHP) != 0) ? " " : "",
					(ITEM_getInt(createitemindex, ITEM_MODIFYCHARM) != 0) ? "概" : "", (ITEM_getInt(createitemindex, ITEM_MODIFYCHARM) > 0) ? "+" : "", (ITEM_getInt(createitemindex, ITEM_MODIFYCHARM) != 0) ? nowcharm : "", (ITEM_getInt(createitemindex, ITEM_MODIFYCHARM) != 0) ? " " : "",
					(ITEM_getInt(createitemindex, ITEM_CRITICAL) != 0) ? "农府" : "", (ITEM_getInt(createitemindex, ITEM_CRITICAL) > 0) ? "+" : "", (ITEM_getInt(createitemindex, ITEM_CRITICAL) != 0) ? nowcriti : "", (ITEM_getInt(createitemindex, ITEM_CRITICAL) != 0) ? " " : "",
					(ITEM_getInt(createitemindex, ITEM_MODIFYAVOID) != 0) ? "雀乔" : "", (ITEM_getInt(createitemindex, ITEM_MODIFYAVOID) > 0) ? "+" : "", (ITEM_getInt(createitemindex, ITEM_MODIFYAVOID) != 0) ? nowyoid : "",
					(ITEM_getInt(createitemindex, ITEM_OTHERDAMAGE) >0) ? "包烹+" : "", (ITEM_getInt(createitemindex, ITEM_OTHERDAMAGE) >0) ? nowpenetration : "", (ITEM_getInt(createitemindex, ITEM_OTHERDEFC) >0) ? "蒋靛+" : "", (ITEM_getInt(createitemindex, ITEM_OTHERDEFC) >0) ? nowshield : "",
					Mname); 
				if (ITEM_getInt(createitemindex, ITEM_TYPE) == 16 || ITEM_getInt(createitemindex, ITEM_TYPE) >= 20 || ITEM_getInt(createitemindex, ITEM_TYPE) == 5) {
				}
				else {
...
int PETSKILL_ITEM_inslay( int charindex, int inslayindex, int itemindex)
{
	...
	sprintf( buf2, "%s|%s|%s|%s|%s|%s|%s|%s|%s|%s"
