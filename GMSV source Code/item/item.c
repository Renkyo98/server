...
INLINE int ITEM_setIntStrict( int index ,ITEM_DATAINT element,int data,int* error)
{
	int buf;
	if(!ITEM_CHECKINDEX(index))
		return *error=FALSE;
				...
				if( strlen( token ) != 0 ) {
					switch(ITEM_itemconfentries[i].type){
						case ITEM_INTENTRY:
							...
							break;
						case ITEM_CHARENTRY:
							strcpysafe( itm.string[ITEM_itemconfentries[i].index].string,sizeof(itm.string[ITEM_itemconfentries[i].index].string),token);
							break;
						case ITEM_INTFUNC:
							{
						...
						case ITEM_CHARFUNC:
							{
								...
								strcpysafe( itm.string[ITEM_itemconfentries[i].index].string,sizeof(itm.string[ITEM_itemconfentries[i].index].string),charfunction( token));
								break;
							}
						default:
							break;
...
#else
	makeEscapeString( ITEM_getChar( itemindex, ITEM_EFFECTSTRING),escapeeffectstring,sizeof(escapeeffectstring));
#endif
...
void Other_DefcharWorkInt( int index)
{
	...
	if( CHAR_getWorkInt( index, CHAR_WORK_ADDATTCRAZED ) > 0 ){
		...
		CHAR_setWorkInt( index, CHAR_WORKFIXDEX, old_dex+old_dex*(temp2/100.0) );
	}
#endif
