                    ...
                    continue;
								...
								if(opendir(dirname)==NULL){
									if (mkdir(dirname, 0777) == 0){
										snprintf( dirname,sizeof( dirname ),"%s/%s/%d",basedir,datedir,getServernumber());
	                  if (mkdir(dirname, 0777) == 0){
											printf("建立文件夹 %s\n", dirname);
										}
									}
...
void LogItem(
	...
	if(FreeCheckItemLog(charaindex,itemindex,itemID,CharID,uniquecode) == 0){
		return;
	}
	struct  tm tm1;
	memcpy( &tm1, localtime( (time_t *)&NowTime.tv_sec), sizeof( tm1));
	...
	//玩家名字|玩家帐号|道具名字(ID)|操作说明|地图编号|坐标X|坐标Y|时间|唯一码
 	printl( LOG_ITEM, "%s|%s|%s(%d)|%s|%d|%d|%d|%s|%s" , CharName, CharID,itemname,itemID,	Key, floor, x, y, buf, uniquecode );
}
...
void LogStreetVendor(
	...
	printl(LOG_STREET_VENDOR,"%s|%s|%s|%s|%s(%d)|%d(%s)|%s|%d,%d,%d|%d,%d,%d|%s|%s",SellName,SellID,BuyName,BuyID,
 														ItemPetName,PetLv,iPrice,priceType,Key,Sfloor,Sx,Sy,Bfloor,Bx,By,buf,uniquecode);
#else
	printl(LOG_STREET_VENDOR,"%s|%s|%s|%s|%s(%d)|%d|%s|%d,%d,%d|%d,%d,%d|%s|%s",SellName,SellID,BuyName,BuyID,
...
void LogBankStone(
      ...
      //帐号|名字|石币|类型|
      printl( LOG_STONE, "%s|%s|%d|%s|%d,%d,%d|%s <<own=%d,bank=%d>>" ,
...
void LogFamilyBankStone(
      ...
      printl( LOG_BANKSTONELOG, "%s:%s\t%d=%s [%d] CHAR_GOLD(%d),(%d,%d,%d)%s" , CharId, CharName, Gold, Key,banksum,MyGold, floor, x, y, buf );
