function FreeFameShow(meindex)
	...
	fmdata[2] = math.floor(char.getInt(meindex, "气势")/100);--2气势个人气势：%-10d
	fmdata[3] = char.getInt(meindex, "活力");--3活力个人活力：%-10d
	fmdata[4] = sasql.getVipPoint(meindex);--4金币
	fmdata[5] = char.getInt(meindex, "积分");--5积分
	fmdata[6] = char.getInt(meindex, "离线时间");--6离线时间：%-10d
	...
	msg = string.format( "疙己：%-10d瘤盔器牢飘：%-10d立加器牢飘：%-10d", fmdata[1], fmdata[4], fmdata[5]);
	...
	msg = string.format( "荤侩备浇：%-10s巢篮矫埃：%-10s", char.getWorkInt( meindex, "经验加成").."%", (lefttime / 60).."盒");
	char.TalkToCli(meindex, -1, msg, 4);
	--[[
	--天数经验卡
